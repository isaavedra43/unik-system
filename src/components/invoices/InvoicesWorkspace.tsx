...
import {
  INVOICE_COLUMNS, INVOICE_COLUMN_MAP, type InvoiceColumnDefinition,
} from '@/modules/invoices/invoices-columns';
import {
  type InvoiceQueryState, type TablePreferenceConfig,
...
import {
  formatCurrency, formatDateOnly, getInvoiceStatusConfig,
...
const VISIBLE_COLUMNS = INVOICE_COLUMNS.filter((c) => c.defaultVisible);
...
export function InvoicesWorkspace({
  ...
  const [data, setData] = useState<InvoicesListResult>(initialData);
  ...
  return (
    <div className="app-content">
      ...
      {previewId && (
        <InvoicePreviewDrawer invoiceId={previewId} basePath={basePath} entityLabel={entityLabel}
...
function formatCell(row: InvoiceListRow, col: InvoiceColumnDefinition): React.ReactNode {
  ...
  if (col.formatter === 'statusDot') {
    const config = getInvoiceStatusConfig(value as string | null);
    return (
...
function WatchToggle({ id, watched, canWatch, watchAction, unwatchAction, onToggle }: {
  ...
  watchAction: InvoicesWorkspaceProps['watchAction'];
  unwatchAction: InvoicesWorkspaceProps['unwatchAction'];
