...
import { type ContactsListResult, type ContactListRow } from '@/modules/contacts/contacts-service';
import {
  formatCurrency,
  formatDateOnly,
...
export interface ContactsWorkspaceProps {
  ...
  initialData: ContactsListResult;
  initialQuery: ContactQueryState;
...
interface SortableHeaderProps {
  columnId: string;
  column: ContactColumnDefinition;
  width: number;
...
function StatusCell({ value, label }: { value: string | null; label: string }) {
  const config = getContactStatusConfig(value);
  if (!value) {
    return (
...
function renderCell(item: ContactListRow, column: ContactColumnDefinition): React.ReactNode {
...
export function ContactsWorkspace({
  ...
  const [query, setQuery] = useState<ContactQueryState>(initialQuery);
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      setPref((prev) => {
        const order =
          prev.columnOrder.length > 0 ? prev.columnOrder : CONTACT_COLUMNS.map((c) => c.id);
  ...
  const addFilter = useCallback((columnId?: string) => {
    ...
    const column = columnId
      ? CONTACT_COLUMN_MAP[columnId]
      : CONTACT_COLUMNS.find((c) => c.filterable && c.type === 'text');
          ...
          <div className="so-toolbar-search">
            <div className="input-with-icon">
              ...
              <input
                className="input"
                type="search"
                placeholder={`Buscar por nombre, empresa, correo, teléfono...`}
                value={query.search ?? ''}
              ...
              {columnManagerOpen ? (
                <div className="so-view-dropdown" style={{ right: 0, left: 'auto', minWidth: 260 }}>
                  <div className="so-column-manager">
                    {CONTACT_COLUMNS.sort((a, b) => a.priority - b.priority).map((col) => (
              ...
              return (
                <div key={i} className="so-filter-row">
                  <select
                    value={rule.field}
                    onChange={(e) => {
                      const newCol = CONTACT_COLUMNS.find((c) => c.field === e.target.value);
                      updateFilter(i, {
                  ...
                  >
                    {CONTACT_COLUMNS.filter((c) => c.filterable).map((c) => (
                    ...
                    <div className="so-filter-values" style={{ display: 'flex', gap: '0.5rem' }}>
                      {col?.type === 'status' ? (
                        ...
                        >
                          ...
                          {getContactStatusOptions().map((opt) => (
                      ...
                      >
                        ...
                        >
                          <input
                            ...
                            aria-label={`Seleccionar ${item.contactName ?? item.id}`}
                            style={{
                              width: '1rem',
                              height: '1rem',
            ...
            <div className="modal-body">
              <div className="form-field">
                ...
                <input
                  ...
                  placeholder="Ej: Mis clientes activos"
                  maxLength={100}
                />
              </div>
              {canShareViews ? (
