...
const VISIBLE_COLUMNS = PRODUCT_COLUMNS.filter((c) => c.defaultVisible);
...
export function ProductsWorkspace({
  ...
  permissionExport: _permissionExport,
  permissionWatch: _permissionWatch,
  permissionShareViews: _permissionShareViews,
  initialData,
  initialQuery,
  ...
  canExport,
  canWatch,
  canShareViews: _canShareViews,
  initialSyncStatus,
  savePreferenceAction: _savePreferenceAction,
  resetPreferenceAction: _resetPreferenceAction,
  createViewAction: _createViewAction,
  watchAction,
  unwatchAction,
  bulkWatchAction: _bulkWatchAction,
  exportAction,
}: ProductsWorkspaceProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const [data, setData] = useState<ProductsListResult>(initialData);
  ...
  const updateUrl = useCallback(
    (updates: { search?: string; page?: number; sort?: typeof sort }) => {
      ...
      if (updates.sort !== undefined) {
        if (updates.sort.length > 0) params.set('sort', JSON.stringify(updates.sort));
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (page !== 1 || search !== (initialQuery.search ?? '')) {
        fetchData();
      }
    }, 350);
    ...
    if (existing) {
      if (existing.direction === 'asc') {
        const newSort = sort.map((s) =>
          s.field === column.field ? { ...s, direction: 'desc' as const } : s
        );
        setSort(newSort);
    ...
    <div className="app-content">
      <div className="mx-auto max-w-[1600px] space-y-4 p-4 sm:p-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {pagination.total} {entityLabelPlural.toLowerCase()}
            </p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={handleSync}
              disabled={syncing}
              className="inline-flex items-center gap-2 rounded-md border border-input bg-background px-3 py-1.5 text-sm font-medium hover:bg-accent disabled:opacity-50 transition-colors"
            >
        ...
        {syncStatus?.latest_run && !syncStatus.active_run && (
          <div className="rounded-md border bg-muted/30 p-3 text-xs text-muted-foreground">
            ...
            {syncStatus.latest_run.completed_at
              ? ` — ${new Date(syncStatus.latest_run.completed_at).toLocaleString('es-MX')}`
              : ''}
            {syncStatus.latest_run.records_seen !== undefined &&
              ` — ${syncStatus.latest_run.records_seen} registros, ${syncStatus.latest_run.details_fetched ?? 0} detalles`}
          </div>
        )}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {loading && rows.length === 0 ? (
                  <tr>
                    <td colSpan={VISIBLE_COLUMNS.length + 1} className="px-3 py-12 text-center text-muted-foreground">
                    ...
                    >
                      ...
                      {canWatch && (
                        <td className="px-2 text-center" onClick={(e) => e.stopPropagation()}>
                          <WatchToggle
                            id={row.id}
                            watched={localWatched.has(row.id)}
                            canWatch={canWatch}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={page <= 1}
                className="inline-flex items-center gap-1 rounded-md border border-input bg-background px-3 py-1.5 text-sm hover:bg-accent disabled:opacity-50 transition-colors"
              >
      ...
      {previewId && (
        <ProductPreviewDrawer
          ...
          canWatch={canWatch}
          isWatched={localWatched.has(previewId)}
...
function formatCell(row: ProductListRow, col: ProductColumnDefinition): React.ReactNode {
  const value = (row as unknown as Record<string, unknown>)[col.id];
  if (value === null || value === undefined) return '—';
  if (col.formatter === 'currency') return formatCurrency(value as string | number, row.currencyCode);
  if (col.formatter === 'date') return formatDateOnly(value as string | Date);
  if (col.formatter === 'statusDot') {
    const config = getProductStatusConfig(value as string | null);
    return (
      <span className="inline-flex items-center gap-1.5">
        <span className={`h-1.5 w-1.5 rounded-full ${
          config.tone === 'success' ? 'bg-success' : config.tone === 'danger' ? 'bg-destructive' : 'bg-muted-foreground'
        }`} />
        {config.label}
      </span>
    );
  }
  if (col.type === 'number') return formatNumber(value as string | number);
  return String(value);
}
...
function WatchToggle({
  ...
  watched: boolean;
  canWatch: boolean;
  watchAction: ProductsWorkspaceProps['watchAction'];
  unwatchAction: ProductsWorkspaceProps['unwatchAction'];
