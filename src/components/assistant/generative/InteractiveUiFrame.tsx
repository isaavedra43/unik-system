...
import { Check, ExternalLink, Maximize2, MessageSquare, X } from 'lucide-react';
...
const CDN_SRC = 'https://cdn.jsdelivr.net https://cdnjs.cloudflare.com https://unpkg.com https://cdn.tailwindcss.com';
...
export function InteractiveUiFrame({
  ...
  useEffect(() => {
    const onMessage = (e: MessageEvent) => {
      ...
      const msg = (e.data as { unikUi?: { action?: string; text?: unknown; url?: unknown; height?: unknown } })?.unikUi;
  ...
  return (
    <div className="gui-card gui-interactive">
      <div className="gui-head">
        <span className="gui-title">{title ?? 'Interfaz interactiva'}</span>
        <span className="gui-source">Aislada · sin acceso a UNIK</span>
        <button
          type="button"
          className="gui-btn gui-interactive-expand"
      ...
      <iframe
        ...
        srcDoc={srcDoc}
        className="gui-interactive-frame"
        style={{ height: expanded ? Math.min(1200, Math.max(480, frameHeight)) : frameHeight }}
      />
      {pending && (
        <div className="gui-pending" role="alert">
          ...
          <span className="gui-pending-actions">
            <button type="button" className="gui-btn gui-btn-primary" onClick={confirm}>
              <Check size={12} /> {pending.kind === 'prompt' ? 'Enviar' : 'Abrir'}
            </button>
