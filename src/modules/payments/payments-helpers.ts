...
const STATUS_MAP: Record<string, PaymentStatusConfig> = {
  success: { raw: 'success', label: 'Exitoso', tone: 'success' },
  deposited: { raw: 'deposited', label: 'Depositado', tone: 'info' },
  refunded: { raw: 'refunded', label: 'Reembolsado', tone: 'warning' },
  failure: { raw: 'failure', label: 'Fallido', tone: 'danger' },
  voided: { raw: 'voided', label: 'Anulado', tone: 'muted' },
...
export function getPaymentStatusLabel(raw: string | null | undefined): string {
  return getPaymentStatusConfig(raw).label;
}

export function getPaymentStatusOptions(): { value: string; label: string }[] {
