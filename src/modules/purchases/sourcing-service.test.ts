...
const meter = (key: string, unit: string) =>
  rows('usageMeter').find(
    (row) =>
      row.dimension === 'extension' &&
      row.key === key &&
      row.unit === unit &&
      row.period === '2026-09-15'
  );
...
/** What the command actually enqueued for a search (the job must run on that payload, not a hand-made one). */
const queuedPayload = (searchId: string) =>
  rows('backgroundJob').find(
    (row) =>
      row.type === 'purchases.sourcing_search' &&
      (row.payload as { searchId?: string })?.searchId === searchId
  )?.payload;
...
const draft = (overrides: Record<string, unknown>) => ({
  ...
  priceSnippets: [],
  evidence: [
    { url: 'https://buscador/x', fetchedAt: NOW.toISOString(), objectId: 'raw_1', sha256: null },
  ],
  confidence: 0.35,
  ...overrides,
});
...
describe('laboratorio de sourcing', () => {
  it('busca una vez, deduplica, detecta proveedores existentes y responde repetidas desde caché sin gastar', async () => {
    ...
    mocks.runBraveSearch.mockResolvedValue({
      candidates: [
        {
          ...draft({ name: 'Acme | Pisos', url: 'https://www.acme.mx/p', domain: 'acme.mx' }),
          dedupeKey: 'dom:acme.mx',
        },
        { ...draft({ name: 'Beta Azulejos', phone: '8199999999' }), dedupeKey: 'tel:8199999999' },
        {
          ...draft({ name: 'Gamma', phone: '+52 81 1234 5678', confidence: 0.7 }),
          dedupeKey: 'tel:8112345678',
        },
      ],
      costUnits: 1,
    ...
    const requested = await purchases.runSourcingSearch(
      people.buyer,
      { query: 'Porcelanato 60x60 Monterrey' },
      { now: NOW }
    );
    expect(requested).toMatchObject({
      status: 'completed',
      data: { cached: false, jobQueued: true, estimatedCostUnits: 1, status: 'pending' },
    });
    const search = rows('sourcingSearch')[0];
    expect(search).toMatchObject({
      providerKey: 'brave_search',
      status: 'pending',
      createdByUserId: 'u_buyer',
    });
    const queued = rows('backgroundJob').filter((row) => row.type === 'purchases.sourcing_search');
    // The reserved day travels with the job so it releases and charges against the period it reserved.
    expect(queued).toEqual([
      expect.objectContaining({
        payload: { searchId: search.id, reservedDay: '2026-09-15' },
        createdBy: 'u_buyer',
      }),
    ]);

    const ran = await runSourcingSearchJob(job(queuedPayload(search.id as string)), { now: NOW });
    ...
    expect(mocks.runBraveSearch).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        searchId: search.id,
        actor: expect.objectContaining({ id: 'u_buyer' }),
      })
    );
    ...
    expect(candidates.find((c) => c.dedupeKey === 'dom:acme.mx')).toMatchObject({
      supplierId: acme.id,
      status: 'new',
    });
    ...
    expect(mocks.notifyUser).toHaveBeenCalledWith(
      expect.objectContaining({
        userId: 'u_buyer',
        type: 'purchase_sourcing_done',
        body: '3 candidato(s), 1 ya son proveedores',
      })
    );

    const cached = await purchases.runSourcingSearch(
      people.buyer,
      { query: '  porcelanato 60X60   monterrey ' },
      { now: NOW }
    );
    ...
    expect(
      rows('backgroundJob').filter((row) => row.type === 'purchases.sourcing_search')
    ).toHaveLength(1);
    expect(mocks.runBraveSearch).toHaveBeenCalledTimes(1);
    expect(unitsSpent()).toBe(1);

    const page = await listSourcingCandidates(people.buyer, {
  ...
  it('presupuesto diario, sitios autorizados y búsquedas fallidas', async () => {
    seedSpent(200);
    expect(
      await purchases.runSourcingSearch(people.buyer, { query: 'adhesivo cerámico' }, { now: NOW })
    ).toMatchObject({
      status: 'rejected',
    ...
    expect(
      await purchases.runSourcingSearch(
        people.buyer,
    ...
    fake.seed('integrationConfig', {
      ...
      isEnabled: true,
      settings: { allowedHosts: ['catalogo.mx'] },
      updatedAt: NOW,
    });
    ...
    expect(allowed).toMatchObject({
      status: 'completed',
      data: { estimatedCostUnits: 2, jobQueued: true },
    });

    mocks.runCatalogPages.mockResolvedValueOnce({
      candidates: [],
      costUnits: 1,
      rawResultObjectId: null,
      error: 'El sitio pidió comprobar que no somos un robot; no se consulta',
      detail: {},
    });
    const search = rows('sourcingSearch')[0];
    await runSourcingSearchJob(job(queuedPayload(search.id as string), 'job_catalog'), {
      now: NOW,
    });
    expect(search).toMatchObject({
      status: 'failed',
    ...
    expect(unitsSpent()).toBe(1);
    expect(mocks.runCatalogPages).toHaveBeenCalledWith(
      'malla',
      ['https://catalogo.mx/malla'],
      expect.anything()
    );
    ...
    expect(
      await purchases.runSourcingSearch(people.outsider, { query: 'malla' }, { now: NOW })
    ).toMatchObject({ status: 'rejected', errorCode: 'forbidden' });
  });

  it('promover candidatos: crea proveedor con sus canales o liga el existente', async () => {
    const acme = seedSupplier(fake, { name: 'Acme Materiales', website: 'https://acme.mx' });
    fake.seed('sourcingCandidate', {
      id: 'cand_beta',
      dedupeKey: 'tel:8199999999',
      name: 'Beta Azulejos',
      phone: '8199999999',
      email: 'ventas@beta.mx',
      url: 'https://beta.mx',
    });
    fake.seed('sourcingCandidate', {
      id: 'cand_acme',
    ...
    fake.seed('contact', {
      zohoContactId: 'z_beta',
      contactType: 'vendor',
      companyName: 'Beta Azulejos SA',
      mobile: '+528199999999',
    ...
    expect(detail.zohoVendor).toEqual({
      zohoContactId: 'z_beta',
      name: 'Beta Azulejos SA',
      reason: 'phone',
    });

    const promoted = await purchases.promoteCandidateToSupplier(
      people.buyer,
      { candidateId: 'cand_beta', paymentMode: 'credit', paymentTermsDays: 15 },
      { now: NOW }
    );
    expect(promoted).toMatchObject({
      ...
      data: {
        created: true,
        supplier: { name: 'Beta Azulejos', zohoContactId: 'z_beta', paymentMode: 'credit' },
      },
    });
    ...
    expect(rows('sourcingCandidate').find((c) => c.id === 'cand_beta')).toMatchObject({
      status: 'promoted',
      supplierId: promoted.data!.supplier.id,
    });

    const linked = await purchases.promoteCandidateToSupplier(
      people.buyer,
    ...
    expect(linked).toMatchObject({
      status: 'completed',
      data: { created: false, supplier: { id: acme.id } },
    });
    expect(rows('supplier')).toHaveLength(2);

    expect(
      await purchases.setCandidateStatus(
        people.buyer,
        { candidateId: 'cand_acme', status: 'rejected' },
        { now: NOW }
      )
    ).toMatchObject({
      status: 'rejected',
    ...
    expect(
      await purchases.createSupplier(
        people.buyer,
        { name: 'Beta Azulejos', channels: [{ type: 'whatsapp', value: '+52 81 9999 9999' }] },
        { now: NOW }
      )
...
describe('presupuesto del laboratorio con búsquedas simultáneas y reintentos', () => {
  it('reserva al encolar, nunca rebasa el día y un reintento vuelve a reservar antes de gastar', async () => {
    fake.seed('integrationConfig', {
      ...
      displayName: 'Laboratorio',
      isEnabled: true,
      settings: { dailyBudgetUnits: 3, allowedHosts: ['catalogo.mx'] },
      updatedAt: NOW,
    });
    const first = await purchases.runSourcingSearch(
      people.buyer,
      { query: 'adhesivo uno' },
      { now: NOW }
    );
    const second = await purchases.runSourcingSearch(
      people.buyer,
      { query: 'adhesivo dos' },
      { now: NOW }
    );
    ...
    const retried = await runSourcingSearchJob(
      { ...job({ searchId }, 'job_retry'), attempt: 2 },
      { now: NOW }
    );
    expect(retried).toMatchObject({ costUnits: 1 });
    expect(unitsSpent()).toBe(3);
    const otherId = second.data!.searchId;
    const exhausted = await runSourcingSearchJob(
      { ...job({ searchId: otherId }, 'job_retry_2'), attempt: 2 },
      { now: NOW }
    );
    expect(exhausted).toMatchObject({ error: 'budget_exhausted', costUnits: 0 });
    expect(mocks.runBraveSearch).toHaveBeenCalledTimes(1);
    expect(rows('sourcingSearch').find((row) => row.id === otherId)).toMatchObject({
      status: 'failed',
    });
    expect(unitsSpent()).toBe(3);
  });
