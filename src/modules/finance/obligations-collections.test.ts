...
const obligationOfOrder = (zohoSalesOrderId: string) =>
  fake
    .rows('obligation')
    .find((o) => o.zohoSalesOrderId === zohoSalesOrderId && o.kind === 'receivable')!;
const settlementsOf = (obligationId: string) =>
  fake.rows('obligationSettlement').filter((s) => s.obligationId === obligationId);
...
describe('obligation contracts', () => {
  it('a purchase payable needs its payment authorization; settlements run onObligationSettled', async () => {
    ...
    const early = await settleObligation(
      team.admin,
      { obligationId, amount: '12000', cashAccountId: bank.id },
      opts()
    );
    ...
    const pending = await settleObligation(
      team.admin,
      { obligationId, amount: '1', cashAccountId: bank.id },
      opts()
    );
    ...
    await decideApproval(
      team.approver,
      { approvalRequestId: auth.data!.approvalRequestId, decision: 'approve' },
      { commandId: 'vote', now: NOW }
    );
    expect(eventsOf(fake, 'finance.payment.authorized')).toHaveLength(1);
    expect(
      fake
        .rows('workItem')
        .find((w) => w.objectType === 'obligation' && w.objectId === obligationId)?.title
    ).toBe('Pagar OB-000001 a Aceros del Norte');

    const partial = await settleObligation(
      team.admin,
      { obligationId, amount: '5000', cashAccountId: bank.id },
      opts()
    );
    ...
    const over = await settleObligation(
      team.admin,
      { obligationId, amount: '7000.01', cashAccountId: bank.id },
      opts()
    );
    expect(over).toMatchObject({ status: 'rejected', errorCode: 'over_settlement' });
    const rest = await settleObligation(
      team.admin,
      { obligationId, amount: '7000', cashAccountId: bank.id },
      opts()
    );
  ...
  it('cancel refuses applied money; write-off clears the balance', async () => {
    const created = await createManualObligation(
      team.admin,
      {
        ...
        counterpartyName: 'Arrendatario',
        description: 'Renta de bodega',
        expectedAmount: '1000',
        dueAt: '2026-06-01',
      },
      opts()
    ...
    const obligationId = created.data!.obligationId;
    expect(rowById(fake, 'obligation', obligationId).categoryId).toBe(
      byKey(fake, 'financeCategory', 'otros_ingresos').id
    );
    await settleObligation(
      team.admin,
      { obligationId, amount: '400', cashAccountId: cash('caja_general').id },
      opts()
    );
    const cancel = await cancelObligationCommand(
      team.admin,
      { obligationId, reason: 'Ya no aplica' },
      opts()
    );
    expect(cancel).toMatchObject({ status: 'rejected', errorCode: 'has_settlements' });
    const writeOff = await writeOffObligation(
      team.admin,
      { obligationId, reason: 'Incobrable tras 90 días' },
      opts()
    );
    ...
    expect(linesOf(fake, writeOff.data!.ledgerEntryId)).toEqual([
      [
        'category',
        byKey(fake, 'financeCategory', 'cuentas_incobrables').id,
        '600.00',
        '0.00',
        null,
      ],
    ...
    const other = await createManualObligation(
      team.admin,
      {
        ...
        description: 'Pago provisional',
        expectedAmount: '2500',
      },
      opts()
    );
    const cancelled = await cancelObligationCommand(
      team.admin,
      { obligationId: other.data!.obligationId, reason: 'Capturado por error' },
      opts()
    );
    ...
    expect(rowById(fake, 'ledgerEntry', entry.reversedByEntryId)).toMatchObject({
      kind: 'reversal',
      reversesEntryId: entry.id,
    });
  });
  it('reprograma la fecha de una obligación abierta sin tocar su asiento', async () => {
    const created = await createManualObligation(
      team.admin,
      {
        kind: 'receivable',
        counterpartyType: 'customer',
        counterpartyName: 'Constructora del Bajío',
    ...
    const obligationId = created.data!.obligationId;
    const entryId = rowById(fake, 'obligation', obligationId).ledgerEntryId;

    const moved = await rescheduleObligation(
      team.admin,
      { obligationId, dueAt: '2026-10-15', reason: 'Renegociado con el proveedor' },
      opts()
    );
    expect(moved.data).toMatchObject({
      previousDueAt: '2026-09-20',
      dueAt: '2026-10-15',
      expectedCashAt: '2026-10-15',
    });
    const row = rowById(fake, 'obligation', obligationId);
    expect(dateKeyOf(row.dueAt)).toBe('2026-10-15');
    expect(dateKeyOf(row.expectedCashAt)).toBe('2026-10-15');
    // Ni asiento nuevo ni reversa: sigue siendo el mismo asiento, vivo.
    expect(row.ledgerEntryId).toBe(entryId);
    expect(rowById(fake, 'ledgerEntry', entryId).reversedByEntryId).toBeFalsy();
    expect(eventsOf(fake, 'finance.obligation.rescheduled')).toHaveLength(1);

    // Una obligación ya liquidada no se reprograma (un cobro no pide autorización).
    const settled = await settleObligation(
      team.admin,
      { obligationId, amount: '3000', cashAccountId: cash('caja_general').id },
      opts()
    );
    expect(settled.data).toMatchObject({ status: 'settled' });
    const late = await rescheduleObligation(
      team.admin,
      { obligationId, dueAt: '2026-11-01', reason: 'Tarde' },
      opts()
    );
    expect(late).toMatchObject({ status: 'rejected', errorCode: 'invalid_state' });
    expect(dateKeyOf(rowById(fake, 'obligation', obligationId).dueAt)).toBe('2026-10-15');
  });
});
...
describe('expected income vs collections', () => {
  it('spreads one payment over two expected receivables, idempotently', async () => {
    ...
    expect(linesOf(fake, ob1.ledgerEntryId)).toEqual([
      ...
      [
        'category',
        byKey(fake, 'financeCategory', 'ventas').id,
        '0.00',
        '3000.00',
        byKey(fake, 'costCenter', 'cc_ventas').id,
      ],
    ]);

    seedCustomerPayment(fake, {
      zohoPaymentId: 'P-1',
      amount: '5000',
      zohoCustomerId: 'C1',
      date: '2026-09-14',
      paymentNumber: 'PAGO-77',
    });
    ...
    expect(
      settlementsOf(ob1.id).map((s) => [s.externalRef, D(s.amount).toFixed(2), s.zohoPaymentId])
    ).toEqual([[`zoho_payment:P-1:${ob1.id}`, '3000.00', 'P-1']]);
    expect(settlementsOf(ob2.id).map((s) => [s.externalRef, D(s.amount).toFixed(2)])).toEqual([
      [`zoho_payment:P-1:${ob2.id}`, '2000.00'],
    ]);
    expect([
      rowById(fake, 'obligation', ob1.id).status,
      rowById(fake, 'obligation', ob2.id).status,
    ]).toEqual(['settled', 'settled']);
    expect(D(cash('banco_zoho').currentBalance).toFixed(2)).toBe('5000.00');
    expect(eventsOf(fake, 'finance.collection.matched')[0].payload).toMatchObject({
      rule: 'fifo',
      zohoPaymentId: 'P-1',
    });
  ...
  it('a payment pair already applied is skipped by its externalRef', async () => {
    ...
    const first = await matchPaymentToObligation(
      team.admin,
      { zohoPaymentId: 'P-9', allocations: [{ obligationId: ob.id, amount: '1000' }] },
      opts()
    );
    expect(first.data?.applied).toHaveLength(1);
    const second = await matchPaymentToObligation(
      team.admin,
      { zohoPaymentId: 'P-9', allocations: [{ obligationId: ob.id, amount: '500' }] },
      opts()
    );
    expect(second.data).toMatchObject({
      applied: [],
      skipped: [{ obligationId: ob.id, reason: 'already_applied' }],
      remaining: '1000.00',
    });
  ...
  it('flags the remainder of an overpayment and records it as unexpected income', async () => {
    ...
    const item = fake
      .rows('workItem')
      .find((w) => w.objectType === 'customer_payment' && w.objectId === 'P-2')!;
    expect(item).toMatchObject({
      title: 'Asignar cobro PAGO-2 de Grupo Delta',
      status: 'open',
      ownerUserId: 'u-conta',
    ...
    const unexpected = await recordUnexpectedCollection(
      team.admin,
      { zohoPaymentId: 'P-2' },
      opts()
    );
    expect(unexpected.data).toMatchObject({ remaining: '0.00', completedWorkItemIds: [item.id] });
    expect(rowById(fake, 'obligation', unexpected.data!.obligationId)).toMatchObject({
      status: 'settled',
      kind: 'receivable',
      counterpartyName: 'Grupo Delta',
    });
  ...
  it('equal balances are ambiguous until a person assigns the payment', async () => {
    ...
    const forbidden = await matchPaymentToObligation(
      viewer,
      { zohoPaymentId: 'P-3', allocations: [{ obligationId: target.id, amount: '1500' }] },
      opts()
    );
  ...
  it('follows the invoices of the payment to their sales order', async () => {
    ...
    const invoice = fake.seed('invoice', {
      zohoInvoiceId: 'INV-8',
      invoiceNumber: 'F-8',
      zohoCustomerId: 'C4',
    });
  ...
  it('a voided order cancels its receivable by reversal, or asks a person when money was applied', async () => {
    const { order } = seedSalesOrderCase(fake, {
      caseId: 'case-10',
      zohoSalesOrderId: 'SO-10',
      total: '800',
      zohoCustomerId: 'C5',
      orderDate: '2026-09-01',
    });
    const paidOrder = seedSalesOrderCase(fake, {
      caseId: 'case-11',
      zohoSalesOrderId: 'SO-11',
      total: '900',
      zohoCustomerId: 'C6',
      orderDate: '2026-09-01',
    });
    ...
    expect(
      fake
        .rows('workItem')
        .find((w) => w.objectType === 'obligation' && w.objectId === obligationOfOrder('SO-11').id)
        ?.title
    ).toContain('Orden anulada con cobro');
    // a voided order never gets a new expected receivable
    seedSalesOrderCase(fake, {
      caseId: 'case-12',
      zohoSalesOrderId: 'SO-12',
      total: '100',
      zohoCustomerId: 'C5',
      orderDate: '2026-09-01',
      status: 'void',
  ...
  it('reversing a settlement frees the payment, which a person re-applies under a new reference', async () => {
    ...
    const notReversal = await reverseSettlement(
      team.approver,
      { obligationId: ob.id, settlementId: settlement.id, reason: 'No' },
      opts()
    );
    expect(notReversal).toMatchObject({ status: 'rejected', errorCode: 'forbidden' });
    const reversed = await reverseSettlement(
      team.admin,
      { obligationId: ob.id, settlementId: settlement.id, reason: 'Pago de otra orden' },
      opts()
    );
    ...
    const twice = await reverseSettlement(
      team.admin,
      { obligationId: ob.id, settlementId: settlement.id, reason: 'Otra vez' },
      opts()
    );
    ...
    const reassigned = await matchPaymentToObligation(
      team.admin,
      { zohoPaymentId: 'P-20', allocations: [{ obligationId: ob.id, amount: '3000' }] },
      opts()
    );
    expect(reassigned.data).toMatchObject({
      remaining: '0.00',
      completedWorkItemIds: [reversed.data!.paymentWorkItemId],
    });
    expect(settlementsOf(ob.id).map((s) => s.externalRef)).toContain(
      `zoho_payment:P-20:${ob.id}#2`
    );
  ...
  it('expects the receivable of a new case through the onCaseStarted listener and the reconciler', async () => {
    registerCollectionsCaseListener();
    const listener = mocks.onCaseStarted.mock.calls[0][0] as (
      event: Record<string, unknown>
    ) => Promise<void>;
    seedSalesOrderCase(fake, {
      caseId: 'case-30',
      zohoSalesOrderId: 'SO-30',
      total: '1200',
      zohoCustomerId: 'C8',
