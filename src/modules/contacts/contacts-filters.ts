import { z } from 'zod';
import { CONTACT_FILTERABLE_FIELDS, CONTACT_SORTABLE_FIELDS } from './contacts-columns';

/**
 * Filter model for the Contacts workspace.
...
const textFilterSchema = z.object({
  field: z.string().refine((f) => CONTACT_FILTERABLE_FIELDS.has(f)),
...
export const contactFilterGroupSchema = z.object({
  ...
  rules: z.array(contactFilterRuleSchema).default([]),
});
...
export const contactQueryStateSchema = z.object({
  search: z.string().optional(),
  filters: contactFilterGroupSchema.default({ logic: 'AND', rules: [] }),
  sort: contactSortSchema.default([]),
  page: z.coerce.number().int().min(1).default(1),
  page_size: z.coerce.number().int().min(1).max(500).default(50),
  /** 'customer' | 'vendor' — server enforces this; client cannot override. */
  contactType: z.enum(['customer', 'vendor']).optional(),
});

export type ContactQueryState = z.output<typeof contactQueryStateSchema>;
