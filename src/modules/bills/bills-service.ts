...
import { BILL_COLUMNS, BILL_COLUMN_MAP } from './bills-columns';
import {
  billFilterGroupSchema,
  billQueryStateSchema,
  type BillFilterGroup,
  type BillQueryState,
  type BillSort,
  DATE_SHORTCUTS,
} from './bills-filters';
...
function buildRuleWhere(
  rule: z.infer<typeof billFilterGroupSchema>['rules'][number]
): Prisma.BillWhereInput {
  const column = BILL_COLUMNS.find((c) => c.field === rule.field);
  if (!column) return {};

  const field = rule.field as keyof Prisma.BillWhereInput;
  ...
  switch (type) {
    case 'text': {
      ...
      if (op === 'is_empty') return { [field]: { equals: null } } as Prisma.BillWhereInput;
      if (op === 'is_not_empty') return { [field]: { not: null } } as Prisma.BillWhereInput;
      if (!val) return {};
      if (op === 'contains')
        return { [field]: { contains: val, mode: 'insensitive' } } as Prisma.BillWhereInput;
      if (op === 'not_contains')
        return {
          [field]: { not: { contains: val, mode: 'insensitive' } },
        } as Prisma.BillWhereInput;
      if (op === 'equals')
        return { [field]: { equals: val, mode: 'insensitive' } } as Prisma.BillWhereInput;
      if (op === 'not_equals')
        return {
          [field]: { not: { equals: val, mode: 'insensitive' } },
        } as Prisma.BillWhereInput;
      if (op === 'starts_with')
        return { [field]: { startsWith: val, mode: 'insensitive' } } as Prisma.BillWhereInput;
      return {};
    }
    case 'status': {
      const op = rule.operator as string;
      ...
      if (op === 'in') {
        ...
        return { [field]: { in: arr, mode: 'insensitive' } } as Prisma.BillWhereInput;
      }
      if (op === 'not_in') {
        const arr = Array.isArray(val) ? val : typeof val === 'string' ? [val] : [];
        if (arr.length === 0) return {};
    ...
    case 'currency': {
      ...
      if (op === 'equals' && num !== null && !Number.isNaN(num))
        return { [field]: { equals: num } } as Prisma.BillWhereInput;
      if (op === 'greater_than' && num !== null && !Number.isNaN(num))
        return { [field]: { gt: num } } as Prisma.BillWhereInput;
      if (op === 'greater_or_equal' && num !== null && !Number.isNaN(num))
        return { [field]: { gte: num } } as Prisma.BillWhereInput;
      if (op === 'less_than' && num !== null && !Number.isNaN(num))
        return { [field]: { lt: num } } as Prisma.BillWhereInput;
      if (op === 'less_or_equal' && num !== null && !Number.isNaN(num))
        return { [field]: { lte: num } } as Prisma.BillWhereInput;
      if (
        op === 'between' &&
        num !== null &&
        numTo !== null &&
        ...
        return { [field]: { gte: num, lte: numTo } } as Prisma.BillWhereInput;
      return {};
    }
    case 'date': {
      const op = rule.operator as string;
      ...
      if (shortcut && DATE_SHORTCUTS.includes(shortcut as (typeof DATE_SHORTCUTS)[number])) {
        ...
        if (range)
          return { [field]: { gte: range.from, lte: range.to } } as Prisma.BillWhereInput;
      }

      const val = rawVal ? parseDate(rawVal as string | Date) : null;
      const valTo = rawValTo ? parseDate(rawValTo as string | Date) : null;
      if (op === 'equals' && val)
        return { [field]: { equals: val } } as Prisma.BillWhereInput;
      if (op === 'before' && val) return { [field]: { lt: val } } as Prisma.BillWhereInput;
      if (op === 'after' && val) return { [field]: { gt: val } } as Prisma.BillWhereInput;
      if (op === 'between' && val && valTo)
        return { [field]: { gte: val, lte: valTo } } as Prisma.BillWhereInput;
      return {};
    }
    case 'boolean': {
      const op = rule.operator as string;
...
function buildFilterWhere(filterGroup: BillFilterGroup): Prisma.BillWhereInput {
...
function buildSearchWhere(search: string | undefined): Prisma.BillWhereInput {
...
function buildSortOrderBy(sort: BillSort): Prisma.BillOrderByWithRelationInput[] {
  if (!sort || sort.length === 0) {
    return [{ date: 'desc' }, { createdAt: 'desc' }, { id: 'desc' }];
  }
  return sort.map(
    (s) => ({ [s.field]: s.direction }) as Prisma.BillOrderByWithRelationInput
  );
}

function buildWhere(query: BillQueryState): Prisma.BillWhereInput {
...
export async function getBillsWorkspace(
  rawQuery: unknown
): Promise<EntityListResult<BillListRow>> {
  const query = billQueryStateSchema.parse(rawQuery);
...
export const EXPORT_COLUMN_IDS = BILL_COLUMNS.map((c) => c.id);

function getExportColumns(includeAll: boolean): typeof BILL_COLUMNS {
  if (includeAll) return BILL_COLUMNS;
  return BILL_COLUMNS.filter((c) => c.defaultVisible);
}

function formatExportValue(row: BillListRow, columnId: string): string {
  const column = BILL_COLUMN_MAP[columnId];
  if (!column) return '';
  ...
  if (column.formatter === 'statusDot') {
    return getBillStatusConfig(value as string | null).label;
  }
  return String(value);
}
...
): Promise<{ rows: BillListRow[]; columns: typeof BILL_COLUMNS }> {
  const query = billQueryStateSchema.parse(rawQuery);
  const where = buildWhere(query);
  const orderBy = buildSortOrderBy(query.sort);
...
export function buildCsv(rows: BillListRow[], columns: typeof BILL_COLUMNS): string {
...
// Re-export helper for convenience consumers of the service.
export { formatNumber };
