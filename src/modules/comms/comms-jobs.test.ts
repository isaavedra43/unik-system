...
vi.mock('@/lib/prisma', () => ({ prisma: { commMessage: { findUnique: h.findUnique, update: vi.fn() } } }));
...
vi.mock('@/modules/realtime/realtime-service', () => ({
  ...
  REALTIME_CHANNELS: { user: (id: string) => `user:${id}`, inbox: (scope: string) => `inbox:${scope}` },
}));
...
vi.mock('@/modules/auth/permissions', () => ({ isKnownPermission: (key: string) => h.knownPermissions.has(key) }));
vi.mock('@/modules/purchases/rfq-service', () => ({ interpretRfqReplyIfTagged: h.interpretRfqReplyIfTagged }));

import { FANOUT_RECEIVER_NOT_INSTALLED, defaultMessageFanoutDeps, runMessageFanout, type MessageFanoutDeps } from './comms-jobs';

function deps(): MessageFanoutDeps & { touchConversation: ReturnType<typeof vi.fn>; interpretRfqReplyIfTagged: ReturnType<typeof vi.fn> } {
  return {
...
describe('runMessageFanout', () => {
  it('interprets inbound replies of RFQ conversations and touches the CRM', async () => {
    h.findUnique.mockResolvedValue(message('inbound', ['vip', 'rfq:rfq_123']));
    const receivers = deps();
    expect(await runMessageFanout('m1', receivers)).toEqual({ messageId: 'm1', crm: 'done', rfq: 'done' });
  ...
  it('only touches the CRM for untagged conversations and outbound messages', async () => {
    ...
    expect(await runMessageFanout('m1', receivers)).toMatchObject({ crm: 'done', rfq: 'not_tagged' });
    h.findUnique.mockResolvedValueOnce(message('outbound', ['rfq:rfq_123']));
  ...
  it('skips a message that no longer exists', async () => {
    ...
    const receivers = deps();
    expect(await runMessageFanout('gone', receivers)).toEqual({ messageId: 'gone', skipped: 'missing', crm: 'skipped', rfq: 'not_tagged' });
  ...
  it('runs both receivers when one fails and throws so the job retries', async () => {
    h.findUnique.mockResolvedValue(message('inbound', ['rfq:rfq_9']));
    const receivers = deps();
    receivers.interpretRfqReplyIfTagged.mockRejectedValue(new Error('modelo no disponible'));
    await expect(runMessageFanout('m1', receivers)).rejects.toThrow('Fan-out incompleto del mensaje m1: RFQ: modelo no disponible');
    expect(receivers.touchConversation).toHaveBeenCalledWith('m1');
...
describe('comms.message_fanout handler', () => {
  it('is registered and calls the CRM and purchases receivers', async () => {
    const registration = h.registerJobHandler.mock.calls.find(([type]) => type === 'comms.message_fanout');
    expect(registration).toBeDefined();
    const handler = registration![1] as (ctx: { payload: { messageId: string }; log: (...args: unknown[]) => void }) => Promise<unknown>;
    h.findUnique.mockResolvedValue(message('inbound', ['rfq:rfq_1']));

    await expect(handler({ payload: { messageId: 'm7' }, log: vi.fn() })).resolves.toEqual({ messageId: 'm7', crm: 'done', rfq: 'done' });
  ...
  it('never loads (nor retries) a receiver whose module permissions are not registered', async () => {
    ...
    await expect(defaultMessageFanoutDeps.touchConversation('m8')).resolves.toEqual(FANOUT_RECEIVER_NOT_INSTALLED);
    await expect(defaultMessageFanoutDeps.interpretRfqReplyIfTagged('m8')).resolves.toEqual(FANOUT_RECEIVER_NOT_INSTALLED);
    h.findUnique.mockResolvedValue(message('inbound', ['rfq:rfq_1']));
