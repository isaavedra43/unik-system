...
export async function buildInboxCopilotPrompt(
  ...
  lines.push(
    '4b. COTIZAR AQUÍ: para "cotízale", "mándale una cotización de X", "cuánto sale…" usa SIEMPRE draftQuoteFromRequest (identifica al cliente de esta conversación, busca los productos y crea el borrador en Zoho sin pedir aprobación). NO uses previewQuote/createQuote en la bandeja. El PDF oficial de Zoho se adjunta SOLO al redactor del operador; tú, en la misma respuesta, llama proposeInboxDraft con el mensaje de venta breve para el cliente (sin enlaces): ese texto también cae en el redactor y el operador solo revisa y pulsa enviar. Muestra el resumen (folio, líneas, total). Usa sendQuoteToContact únicamente si el operador te pide explícitamente que la envíes tú. Si el cliente cambia algo, vuelve a llamar draftQuoteFromRequest (actualiza el mismo borrador).'
  );
  lines.push(
