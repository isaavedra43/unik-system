...
const SYNONYMS: Record<string, string[]> = {
  ...
  // Operations layer (expedientes, solicitudes entre áreas, incidencias, Mi trabajo, Control Tower)
  case: ['expediente', 'expedientes', 'caso', 'casos', 'sala'],
  cases: ['expedientes', 'expediente', 'casos', 'caso'],
  area: ['area', 'areas', 'departamento', 'equipo'],
  incident: ['incidencia', 'incidencias', 'problema', 'falla', 'reclamo'],
  escalate: ['escalar', 'escala', 'escalalo', 'urgente', 'supervisor'],
  carrier: ['transportista', 'paqueteria', 'fletera', 'flete', 'transporte'],
  count: ['conteo', 'conte', 'contar', 'cuantas hay', 'danadas'],
  stuck: ['atorado', 'atorados', 'trabado', 'trabados', 'detenido', 'sin avance'],
  blocking: ['bloquea', 'bloqueando', 'atora', 'traba', 'detiene', 'bloqueo'],
  who: ['quien', 'quienes'],
  production: ['produccion', 'corte', 'merma', 'fabricacion', 'manufactura'],
  expense: ['gasto', 'gastos', 'caja', 'egreso', 'comprobante'],
  obligation: ['obligacion', 'obligaciones', 'pago pendiente', 'por pagar'],
  opportunity: ['oportunidad', 'oportunidades', 'radar', 'prospecto'],
  sourcing: ['proveedor', 'proveedores', 'cotizacion de proveedor', 'abastecimiento'],
  research: ['investiga', 'investigar', 'buscar proveedor', 'compara proveedores'],
  items: ['pendientes', 'tareas', 'trabajos'],
  item: ['pendiente', 'tarea', 'trabajo'],
  snapshot: ['estado', 'situacion', 'como va', 'avance'],
  explain: ['explica', 'explicame', 'por que', 'porque'],
  responsible: ['responsable', 'encargado', 'quien atiende', 'suplente'],
  pulse: ['pulso', 'panorama', 'como va la empresa'],
  company: ['empresa', 'operacion', 'general'],
  delay: ['retraso', 'atraso', 'demora', 'se retrasa'],
  simulate: ['simula', 'simular', 'que pasa si'],
  acknowledge: ['recibido', 'enterado', 'acusa'],
  respond: ['responde', 'acepta', 'aceptar', 'bloquea', 'rechaza', 'resuelve'],
  complete: ['completa', 'completar', 'termina', 'cierra', 'listo'],
  assign: ['asigna', 'asignar', 'reparte', 'responsable'],
  reserve: ['aparta', 'apartar', 'reserva', 'reservar'],
  verification: ['verifica', 'verificacion', 'conteo', 'revisar fisicamente'],
  record: ['registra', 'registrar', 'anota', 'captura'],
  post: ['publica', 'anota', 'escribe en la sala'],
  day: ['dia', 'hoy', 'jornada'],
  my: ['mi', 'mis', 'me toca'],
  open: ['abre', 'abrir', 'levanta', 'reporta'],
  authorize: ['autoriza', 'autorizar', 'aprueba', 'libera'],
  propose: ['propone', 'propuesta', 'sugiere', 'plan'],
};
...
/** Tools of the operations layer (expediente, solicitudes, incidencias, Mi trabajo, Control Tower). */
export const OPERATIONS_TOOL_NAMES: readonly string[] = [
  'getCaseSnapshot',
  'explainCase',
  'listAreaWorkItems',
  'findResponsible',
...
/**
 * Operations vocabulary (normalized text: no accents, lowercase): expediente/caso/sala, solicitud,
 * escalar, incidencia, transportista/paquetería/fletera, conteo, atorado/trabado,
 * compra/proveedor/cotización de proveedor, producción/corte/merma, gasto/caja/obligación, oportunidad/radar,
 * viaje/parada/despacho/chofer (logística).
 */
const OPERATIONS_DOMAIN_TEST =
  /expedient|\bcasos?\b|\bsalas?\b|solicitud|\bescala(r|lo|la|do|da|ndo|miento)?\b|\bescalen?\b|incidenc|transportist|paqueteri|fletera|\bfletes?\b|\bconteos?\b|\bconte\b|atorad|trabad|compra|proveedor|produccion|\bcortes?\b|merma|\bgastos?\b|\bcaja\b|obligacion|oportunidad|\bradar\b|\bviajes?\b|\bparadas?\b|despacho|chofer/;

/** Coarse intent → categories/tools boost. Regexes run on the normalized message. */
const DOMAIN_RULES: DomainRule[] = [
  {
    domain: 'quotes',
    test: /\bcotiz|presupuest|estimate/,
    categories: [],
    tools: [
      'queryQuotes',
      'getQuoteDetail',
  ...
  {
    domain: 'sales',
    test: /\bvent|\borden|pedido|vendi|efectivo|corte|caja|pie de obra|entreg|reparto/,
    categories: ['sales'],
    tools: [
      'querySalesOrders',
      'getSalesOrderDetail',
  ...
  {
    domain: 'operations',
    test: OPERATIONS_DOMAIN_TEST,
    categories: ['operations'],
    tools: [...OPERATIONS_TOOL_NAMES],
  },
  {
    domain: 'invoices',
    test: /factur|cfdi|timbr/,
    categories: ['invoices'],
    tools: [
      'queryInvoices',
      'getInvoiceDetail',
  ...
  {
    domain: 'payments',
    test: /\bpago|\bcobr|abono|saldo|adeud|deud|deben|moros|vencid|antigued/,
  ...
  {
    domain: 'purchases',
    test: /compra|proveedor|\bbill|gasto/,
  ...
  {
    domain: 'inventory',
    test: /inventar|existenc|stock|\bproduct|articul|material|catalog|\bsku\b|loseta|piel|bajo stock|agot/,
  ...
  {
    domain: 'customers',
    test: /client|contact|expedient|telefon|correo|inactiv|reactiv|salud|riesgo/,
    categories: ['contacts'],
    tools: [
      'queryContacts',
      'getContactDetail',
  ...
  {
    domain: 'packages',
    test: /paquet|env[ií]o|guia|paqueter|transport|rastre/,
    categories: ['packages'],
    tools: ['queryPackages', 'getPackageDetail', 'notifyDelayedDeliveries'],
  },
  {
    domain: 'analytics',
    test: /tendenc|pronost|proyecc|compar|versus|\bvs\b|ranking|\btop\b|mejores|kpi|indicador|metric|desempe|rendimient|anomal|analiz|analisis|retenc|segment|patron|horari|ritmo|crec|cay|baj[oó]|subi/,
  ...
  {
    domain: 'documents',
    test: /reporte|informe|\bpdf\b|excel|xlsx|word|docx|\bcsv\b|grafic|tabla|imagen|foto|export|descarg|archivo|documento/,
    categories: ['export'],
    tools: [
      'composeDocument',
      'generatePdfReport',
  ...
  {
    domain: 'messaging',
    test: /mensaje|whatsapp|\bsms\b|manda|envia|escribe|avisa|dile|contesta|respond|recordatorio|masivo|campa/,
  ...
  {
    domain: 'chat',
    test: /chat|canal|equipo|compa[ñn]er|interno|fija|pinea|reunion|evento|agenda|junta/,
    categories: [],
    tools: [
      'listChatChannels',
  ...
  {
    domain: 'calls',
    test: /llam|marc[aá]|telefon|habl[ae]|\bvoz\b|grabaci|transcrip/,
    categories: [],
    tools: [
      'callContact',
      'startInternalCall',
  ...
  {
    domain: 'pickup',
    test: /recog|ubicaci|direccion|bodega|mapa|maps|donde estan|horario/,
    categories: [],
    tools: ['getPickupLocation', 'sendMessageToContact'],
  },
  {
    domain: 'knowledge',
    test: /manual|politic|procedim|biblioteca|conocimient|ficha|garantia|instalaci|especificaci|que dice|promoci|catalogo|folleto|lista de precio/,
    categories: ['knowledge'],
    tools: ['searchKnowledgeLibrary', 'listAttachableDocuments', 'findShareableDocument'],
  },
  {
    domain: 'memory',
    test: /recuerd|memoria|olvid|preferenc|anota|apunta/,
    categories: [],
    tools: ['rememberForUser', 'listUserMemory', 'forgetMemory'],
  },
  {
    domain: 'skills',
    test: /skill|receta|rutina|automatiz/,
    categories: ['skill'],
    tools: ['listSkills', 'runSkill', 'getSkillRunStatus'],
  },
  {
    domain: 'digest',
    test: /como voy|mi dia|jornada|que hice|digest|ponme al dia|resumen del dia|pendientes de hoy/,
    categories: [],
    tools: ['getWorkDigest', 'getRecentActivity', 'getNotifications', 'getDealBlockers'],
  },
  {
    domain: 'attachments',
    test: /adjunt|subi|archivo|factura|recibo|extrae|\bocr\b|lee el|leer el|documento/,
    categories: [],
    tools: [
      'listConversationAttachments',
      'readAttachment',
  ...
  {
    domain: 'system',
    test: /integraci|zoho|sincroniz|notificaci|modulo|sistema|quien soy|permiso/,
    categories: ['system'],
    tools: ['getIntegrationStatus', 'getNotifications', 'getModuleList', 'getCurrentUserContext'],
  },
  {
    domain: 'planning',
    test: /\bplan\b|planea|paso a paso|primero.*luego|trimestral|anual|completo|integral/,
    categories: [],
    tools: ['proposePlan'],
  },
];
...
export function findToolsByTopic<T extends SelectableTool>(
  tools: T[],
  topic: string,
  limit = 30
): T[] {
  ...
  const scored = tools
    .map((tool, index) => ({
      tool,
      index,
      score: scoreTool(tool, topicStems, domainRules, new Set()),
    }))
    .filter((s) => s.score > 0);
