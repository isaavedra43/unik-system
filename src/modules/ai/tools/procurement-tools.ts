...
function registerProcurementTool(def: Omit<ToolDefinition, 'category' | 'enabledByDefault' | 'contextTags'>): void {
  registerTool({ category: 'purchases', enabledByDefault: true, contextTags: CONTEXT_TAGS, ...def });
}

type ScopeContext = Pick<ToolExecutionContext, 'agentAreaKey' | 'agentOnBehalfOfUserId' | 'agentCaseId' | 'approvedProposalId'> | undefined;
...
const listRequestsParams = z.object({
  status: z.enum(['open', 'draft', 'consolidated', 'sourcing', 'ordered', 'closed', 'cancelled']).describe('open = todas las pendientes').optional(),
...
registerProcurementTool({
  ...
  description: 'Lista solicitudes de compra (folio SC-, estado, expediente, partidas con cantidad pedida, ordenada y recibida).',
...
registerProcurementTool({
  ...
  description: 'Busca proveedores de UNIK con su calificación, condiciones de pago, plazo y último precio del artículo (si se indica).',
  requiredPermission: 'purchases.view',
  effect: 'read',
  ...
  execute: async (actor, raw, ctx) => {
    ...
    const page = await listSuppliers(actor, { search: args.query ?? null, zohoItemId: args.zohoItemId ?? null, status: 'active', pageSize: args.limit });
    const prices = args.zohoItemId
      ? await prisma.supplierProduct.findMany({
          ...
          select: { supplierId: true, lastPrice: true, currency: true, unit: true, leadTimeDays: true, lastQuotedAt: true },
        })
      : [];
    return {
      total: page.total,
      suppliers: page.rows.map((s) => {
        ...
        return {
          ...
          ...(price
            ? {
                lastPrice: price.lastPrice ? `${formatMoney(price.lastPrice.toString(), price.currency)} por ${price.unit}` : null,
...
const listCandidatesParams = z.object({
  ...
  status: z.enum(['new', 'contacted', 'rfq_sent', 'quoted', 'approved', 'rejected', 'promoted']).optional(),
...
registerProcurementTool({
  ...
  execute: async (actor, raw, ctx) => {
    ...
    return {
      ...
      note: comparison.ranking.length === 0 ? 'Aún no hay respuestas que comparar' : 'Pesos: costo 50 %, tiempo 25 %, riesgo 15 %, especificación 10 %',
    };
  },
});
...
const sourcingParams = z.object({
  query: z.string().trim().min(3).max(200).describe('Qué buscar (producto, material, especificación, ciudad)'),
  providerKey: z.enum(['brave_search', 'catalog_page']).describe('brave_search = web; catalog_page = páginas autorizadas').default('brave_search'),
  urls: z.array(z.string().trim().max(500)).max(5).describe('Páginas de catálogo (sólo sitios autorizados)').optional(),
...
registerProcurementTool({
  ...
  execute: async (actor, raw, ctx) => {
    ...
    const result = unwrapCommand(
      await runSourcingSearch(actor, input, { commandId: creationCommandId('runSourcingSearch', actor.id, input, ctx) })
    );
...
const draftRfqParams = z.object({
  ...
  requestLineIds: z.array(idArg).max(50).describe('Partidas de solicitudes de compra a cotizar').optional(),
  lines: z
    .array(
      z.object({
...
registerProcurementTool({
  name: 'draftRfq',
  description: 'Crea el BORRADOR de una solicitud de cotización (RFQ) con líneas de solicitudes de compra o libres. No envía nada: para enviarla usa sendRfq.',
  ...
  execute: async (actor, raw, ctx) => {
    ...
    const lines = [
      ...
      ...(args.lines ?? []).map((line) => ({ description: line.description, qty: line.qty, unit: line.unit, zohoItemId: line.zohoItemId ?? null })),
    ];
    if (lines.length === 0) throw new OperationsToolError('Indica las partidas o líneas a cotizar', 'invalid_args');
    const { createRfq } = await commands();
    const input = { title: args.title, dueAt: args.dueAt ?? null, lines };
    const result = unwrapCommand(await createRfq(actor, input, { commandId: creationCommandId('draftRfq', actor.id, input, ctx) }));
...
const sendRfqParams = z.object({
  ...
  channel: z.enum(['whatsapp', 'sms', 'telegram']).describe('Canal preferido (por omisión WhatsApp)').optional(),
...
const CHANNEL_NAMES: Record<string, string> = { whatsapp: 'WhatsApp', sms: 'SMS', telegram: 'Telegram' };
...
registerProcurementTool({
  ...
  summarize: (raw) => {
    ...
    const names = a.recipients?.length ? `: ${a.recipients.slice(0, 5).map((n) => truncateText(n, 40)).join(', ')}${a.recipients.length > 5 ? '…' : ''}` : '';
  ...
  prepareArgs: async (actor, raw, ctx) => {
    ...
    if (!args.supplierIds?.length && !args.candidateIds?.length) return { error: 'Indica al menos un proveedor o candidato' };
    const rfq = await prisma.rfq.findUnique({ where: { id: args.rfqId }, select: { number: true, status: true } });
    if (!rfq) return { error: 'No se encontró la cotización' };
    if (!['draft', 'sent', 'collecting'].includes(rfq.status)) return { error: 'La cotización ya no recibe proveedores' };
    const [suppliers, candidates] = await Promise.all([
      prisma.supplier.findMany({ where: { id: { in: args.supplierIds ?? [] } }, select: { id: true, name: true } }),
      prisma.sourcingCandidate.findMany({ where: { id: { in: args.candidateIds ?? [] } }, select: { id: true, name: true } }),
    ]);
    if (suppliers.length !== (args.supplierIds?.length ?? 0)) return { error: 'Algún proveedor no existe' };
    if (candidates.length !== (args.candidateIds?.length ?? 0)) return { error: 'Algún candidato no existe' };
    return { args: { ...args, rfqNumber: rfq.number, recipients: [...suppliers, ...candidates].map((row) => row.name) } };
  },
  execute: async (actor, raw, ctx) => {
    const args = raw as SendRfqArgs;
    await assertActsForCompras(actor, ctx);
    const invitees = [
      ...(args.supplierIds ?? []).map((supplierId) => ({ supplierId, channel: args.channel ?? null })),
      ...(args.candidateIds ?? []).map((candidateId) => ({ candidateId, channel: args.channel ?? null })),
    ];
...
registerProcurementTool({
  ...
  summarize: (raw) => {
    ...
    return a.invitationId ? `Interpretar la respuesta de la invitación ${a.invitationId}` : `Interpretar las respuestas de la cotización ${a.rfqId}`;
  },
  execute: async (actor, raw, ctx) => {
    const args = raw as z.output<typeof interpretParams>;
    await assertActsForCompras(actor, ctx);
    if (!args.invitationId && !args.rfqId) throw new OperationsToolError('Indica la invitación o la cotización', 'invalid_args');
    ...
    if (invitations.length === 0) throw new OperationsToolError('No hay invitaciones enviadas con conversación que interpretar', 'not_found');
    ...
    for (const invitation of invitations) {
      ...
      if ('reason' in outcome) {
        results.push({ invitationId: invitation.id, status: 'skipped', reason: outcome.reason === 'no_reply' ? 'Sin respuesta del proveedor' : outcome.reason });
      } else {
        results.push({
...
const orderDraftParams = z.object({
  ...
  deliveryMode: z
    ...
    .describe('Por omisión: directo al cliente si las partidas son de una entrega directa, si no a bodega')
    .optional(),
  directDeliveryCaseId: z.string().trim().max(120).describe('Expediente si el proveedor entrega directo al cliente').optional(),
...
registerProcurementTool({
  ...
  summarize: (raw) => {
    ...
    const total = a.lines.reduce((sum, l) => sum + l.qty * l.unitPrice * (1 + (l.taxRate ?? 0.16)), 0);
  ...
  prepareArgs: async (actor, raw, ctx) => {
    ...
    const reason = await canActForArea(actor, 'compras', ctx);
    if (reason) return { error: reason };
    const supplier = await prisma.supplier.findUnique({ where: { id: args.supplierId }, select: { name: true, status: true } });
    if (!supplier) return { error: 'No se encontró el proveedor' };
    if (supplier.status !== 'active') return { error: `${supplier.name} está bloqueado o archivado` };
    return { args: { ...args, supplierName: supplier.name } };
  },
  execute: async (actor, raw, ctx) => {
    const args = raw as OrderDraftArgs;
    await assertActsForCompras(actor, ctx);
    const directCase = args.directDeliveryCaseId ? await resolveCase(args.directDeliveryCaseId) : null;
    const input = {
    ...
    const result = unwrapCommand(
      await createProcurementOrder(actor, input, { commandId: creationCommandId('createProcurementOrderDraft', actor.id, input, ctx) })
    );
...
registerProcurementTool({
  ...
  prepareArgs: async (actor, raw, ctx) => {
    ...
    if (order.status !== 'draft') return { error: 'Sólo se envía a aprobación una orden en borrador' };
    ...
    const result = unwrapCommand(
      await submitProcurementOrder(
        actor,
        {
          ...
          note: args.note ?? null,
          ...(actor.isBot === true && ctx.agentCausedByUserId ? { causedByUserId: ctx.agentCausedByUserId } : {}),
        },
  ...
  lines: z
    .array(
      z.object({
        ...
        differenceKind: z.enum(['short', 'over', 'damaged', 'wrong_item']).describe('Sólo si hay diferencia declarada').optional(),
      })
    )
    .min(1)
    .max(50),
...
registerProcurementTool({
  ...
  prepareArgs: async (actor, raw, ctx) => {
    ...
    const reason = await canActForArea(actor, 'compras', ctx);
    if (reason) return { error: reason };
    const order = await prisma.procurementOrder.findUnique({ where: { id: args.orderId }, select: { number: true, status: true, deliveryMode: true } });
    if (!order) return { error: 'No se encontró la orden de compra' };
    if (order.deliveryMode === 'direct_to_customer') return { error: 'Esta orden la entrega el proveedor directo al cliente' };
    ...
    if (lines.length !== new Set(args.lines.map((l) => l.orderLineId)).size) return { error: 'Alguna partida no pertenece a la orden' };
    const lineSummary = args.lines
      ...
      .join('; ');
    return { args: { ...args, orderNumber: order.number, lineSummary: truncateText(lineSummary, 600) } };
  },
  execute: async (actor, raw, ctx) => {
    const args = raw as ReceiptArgs;
    await assertActsForCompras(actor, ctx);
    ...
    const result = unwrapCommand(await recordGoodsReceipt(actor, input, { commandId: transitionCommandId('recordGoodsReceipt', ctx) }));
