 ...
 * itemsSyncIntervalMs. IntegrationSyncRun is the durable source of truth, so a
...
const SCHEDULER_STATE_KEY = '__unikZohoItemsSchedulerState' as const;
 ...
 * one is older than itemsSyncIntervalMs. Only `mode = 'sync'` runs count: baseline
...
export async function isSyncDue(now: Date = new Date()): Promise<boolean> {
  ...
  return now.getTime() - lastCompletedSync.completedAt.getTime() >= settings.itemsSyncIntervalMs;
}
...
export async function runSchedulerCheck(): Promise<void> {
  ...
  try {
    ...
    log({
      event: 'zoho.items.scheduler.tick_skipped',
  ...
  try {
    ...
    if (await isSyncCoolingDown()) {
      log({
        event: 'zoho.items.scheduler.sync_skipped',
        reason: 'failed_retry_cooldown',
      });
      return;
    }

    log({ event: 'zoho.items.scheduler.due' });
    const startedAt = Date.now();
    const settings = await getIntegrationSettings(INTEGRATION_SOURCE_ZOHO);

    if (!settings.itemsSyncEnabled) {
      return;
    }

    log({ event: 'zoho.items.scheduler.sync_started' });

    const result = await syncItems({
      mode: 'sync',
      maxDetailFetches: settings.itemsMaxDetailFetches,
    });

    log({
      event: 'zoho.items.scheduler.sync_completed',
      runId: result.runId,
    ...
    log({
      event: 'zoho.items.scheduler.sync_failed',
 ...
 */
export async function startItemsScheduler(): Promise<void> {
  const state = getSchedulerState();
  ...
  try {
    ...
    log({
      event: 'zoho.items.scheduler.start_failed',
      reason: error instanceof Error ? error.message : 'db_unreachable',
    });
    return;
  }

  if (!enabled || !settings.itemsSyncEnabled) {
    log({ event: 'zoho.items.scheduler.disabled' });
    return;
  }
  ...
  log({
    event: 'zoho.items.scheduler.started',
    startupDelayMs: settings.startupDelayMs,
    checkIntervalMs: settings.checkIntervalMs,
    itemsSyncIntervalMs: settings.itemsSyncIntervalMs,
    maxDetailFetches: settings.itemsMaxDetailFetches,
  });
}
