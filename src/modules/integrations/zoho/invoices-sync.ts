...
export const ENTITY_TYPE = 'invoice';
...
export class SyncAlreadyRunningError extends Error {
  constructor() {
    super('A invoices sync is already running in this instance');
...
export class SyncFailedError extends Error {
  constructor(
    ...
    super(`Invoices sync failed with code ${errorCode}`);
    this.name = 'SyncFailedError';
  }
}

export const syncInvoicesOptionsSchema = z.object({
  mode: z.enum(['scan', 'sync']).default('scan'),
  maxDetailFetches: z.number().int().min(1).max(200).default(50),
});

export type SyncInvoicesOptions = z.input<typeof syncInvoicesOptionsSchema>;
export type SyncMode = z.output<typeof syncInvoicesOptionsSchema>['mode'];
...
const SYNC_LOCK_KEY = '__unikZohoInvoicesSyncLock' as const;
...
const invoiceSummarySchema = z.object({
  invoice_id: z.union([z.string().min(1), z.number()]).transform((value) => String(value)),
...
const invoiceDetailTimestampSchema = z.object({
  invoice: z.object({ last_modified_time: remoteTimestampSchema }),
});

function extractDetailModifiedAt(rawDetail: unknown): Date | null {
  const parsed = invoiceDetailTimestampSchema.safeParse(rawDetail);
  return parsed.success ? parsed.data.invoice.last_modified_time : null;
}
...
async function scanAllPages(params: SyncParams): Promise<ScanOutcome> {
  ...
  for (;;) {
    ...
    const rawPage = await listInvoices({ page, perPage: params.perPage });
    apiCalls += 1;

    const parsed = invoicesListSchema.safeParse(rawPage);
    if (!parsed.success) {
    ...
    for (const summary of parsed.data.invoices) {
      await recordSummary(summary.invoice_id, summary.last_modified_time);
      recordsSeen += 1;
    }
...
async function fetchPendingDetails(
  ...
  for (const entity of pending) {
    ...
    try {
      const rawDetail = await getInvoice(entity.externalId);
      apiCalls += 1;
...
/**
 * Runs one controlled Invoices synchronization.
 *
 ...
 */
export async function syncInvoices(options: SyncInvoicesOptions = {}): Promise<SyncInvoicesResult> {
  const { mode, maxDetailFetches } = syncInvoicesOptionsSchema.parse(options);
  ...
  const timeoutId = setTimeout(() => {
    ...
    console.error(
      JSON.stringify({
        event: 'zoho.invoices.sync.timeout',
  ...
  try {
    ...
    if (mode === 'sync') {
      try {
        const { normalizePendingInvoiceSnapshots } = await import('@/modules/sales/invoices-normalizer');
        await withTimeout(normalizePendingInvoiceSnapshots({ limit: 100 }), 30_000);
      } catch (error) {
        console.error(
          JSON.stringify({
            event: 'zoho.invoices.sync.normalization_failed',
            runId: run.id,
    ...
    console.info(
      JSON.stringify({
        event: 'zoho.invoices.sync.completed',
        runId: run.id,
        mode,
        pagesScanned: result.pagesScanned,
        recordsSeen: result.recordsSeen,
    ...
    console.error(
      JSON.stringify({
        event: 'zoho.invoices.sync.failed',
        runId: run?.id ?? 'unknown',
        mode,
        errorCode,
        durationMs: completedAt.getTime() - startedAt.getTime(),
