...
import { getVendor, listVendors } from './contacts';
import {
...
export const ENTITY_TYPE = 'vendor';
...
export class SyncAlreadyRunningError extends Error {
  constructor() {
    super('A vendors sync is already running in this instance');
...
export class SyncFailedError extends Error {
  constructor(
    ...
    super(`Vendors sync failed with code ${errorCode}`);
    this.name = 'SyncFailedError';
  }
}

export const syncVendorsOptionsSchema = z.object({
  mode: z.enum(['scan', 'sync']).default('scan'),
  maxDetailFetches: z.number().int().min(1).max(200).default(50),
});

export type SyncVendorsOptions = z.input<typeof syncVendorsOptionsSchema>;
export type SyncMode = z.output<typeof syncVendorsOptionsSchema>['mode'];
...
const SYNC_LOCK_KEY = '__unikZohoVendorsSyncLock' as const;
...
const vendorSummarySchema = z.object({
  contact_id: z.union([z.string().min(1), z.number()]).transform((value) => String(value)),
  last_modified_time: remoteTimestampSchema,
});

const vendorsListSchema = z.object({
  contacts: z.array(vendorSummarySchema),
  page_context: z
    .object({
...
const vendorDetailTimestampSchema = z.object({
  contact: z.object({ last_modified_time: remoteTimestampSchema }),
});

function extractDetailModifiedAt(rawDetail: unknown): Date | null {
  const parsed = vendorDetailTimestampSchema.safeParse(rawDetail);
  return parsed.success ? parsed.data.contact.last_modified_time : null;
}
...
async function scanAllPages(params: SyncParams): Promise<ScanOutcome> {
  ...
  for (;;) {
    ...
    const rawPage = await listVendors({ page, perPage: params.perPage });
    apiCalls += 1;

    const parsed = vendorsListSchema.safeParse(rawPage);
    if (!parsed.success) {
    ...
    for (const summary of parsed.data.contacts) {
      await recordSummary(summary.contact_id, summary.last_modified_time);
      recordsSeen += 1;
    }
...
async function fetchPendingDetails(
  ...
  for (const entity of pending) {
    ...
    try {
      const rawDetail = await getVendor(entity.externalId);
      apiCalls += 1;
...
/**
 * Runs one controlled Vendors synchronization.
 *
 ...
 */
export async function syncVendors(options: SyncVendorsOptions = {}): Promise<SyncVendorsResult> {
  const { mode, maxDetailFetches } = syncVendorsOptionsSchema.parse(options);
  ...
  const timeoutId = setTimeout(() => {
    ...
    console.error(
      JSON.stringify({
        event: 'zoho.vendors.sync.timeout',
  ...
  try {
    ...
    if (mode === 'sync') {
      try {
        const { normalizePendingVendorSnapshots } = await import('@/modules/sales/vendors-normalizer');
        await withTimeout(normalizePendingVendorSnapshots({ limit: 100 }), 30_000);
      } catch (error) {
        console.error(
          JSON.stringify({
            event: 'zoho.vendors.sync.normalization_failed',
            runId: run.id,
    ...
    const result: SyncVendorsResult = {
      runId: run.id,
      mode,
    ...
    console.info(
      JSON.stringify({
        event: 'zoho.vendors.sync.completed',
        runId: run.id,
        mode,
        pagesScanned: result.pagesScanned,
        recordsSeen: result.recordsSeen,
    ...
    console.error(
      JSON.stringify({
        event: 'zoho.vendors.sync.failed',
        runId: run?.id ?? 'unknown',
        mode,
        errorCode,
        durationMs: completedAt.getTime() - startedAt.getTime(),
