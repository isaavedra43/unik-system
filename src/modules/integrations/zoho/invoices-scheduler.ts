 ...
 * invoicesSyncIntervalMs. IntegrationSyncRun is the durable source of truth, so a
...
const SCHEDULER_STATE_KEY = '__unikZohoInvoicesSchedulerState' as const;
 ...
 * one is older than invoicesSyncIntervalMs. Only `mode = 'sync'` runs count: baseline
...
export async function isSyncDue(now: Date = new Date()): Promise<boolean> {
  ...
  return now.getTime() - lastCompletedSync.completedAt.getTime() >= settings.invoicesSyncIntervalMs;
}
...
export async function runSchedulerCheck(): Promise<void> {
  ...
  try {
    ...
    log({
      event: 'zoho.invoices.scheduler.tick_skipped',
  ...
  try {
    ...
    if (await isSyncCoolingDown()) {
      log({
        event: 'zoho.invoices.scheduler.sync_skipped',
        reason: 'failed_retry_cooldown',
      });
      return;
    }

    log({ event: 'zoho.invoices.scheduler.due' });
    const startedAt = Date.now();
    const settings = await getIntegrationSettings(INTEGRATION_SOURCE_ZOHO);

    if (!settings.invoicesSyncEnabled) {
      return;
    }

    log({ event: 'zoho.invoices.scheduler.sync_started' });

    const result = await syncInvoices({
      mode: 'sync',
      maxDetailFetches: settings.invoicesMaxDetailFetches,
    });

    log({
      event: 'zoho.invoices.scheduler.sync_completed',
      runId: result.runId,
    ...
    log({
      event: 'zoho.invoices.scheduler.sync_failed',
 ...
 */
export async function startInvoicesScheduler(): Promise<void> {
  const state = getSchedulerState();
  ...
  try {
    ...
    log({
      event: 'zoho.invoices.scheduler.start_failed',
      reason: error instanceof Error ? error.message : 'db_unreachable',
    });
    return;
  }

  if (!enabled || !settings.invoicesSyncEnabled) {
    log({ event: 'zoho.invoices.scheduler.disabled' });
    return;
  }
  ...
  log({
    event: 'zoho.invoices.scheduler.started',
    startupDelayMs: settings.startupDelayMs,
    checkIntervalMs: settings.checkIntervalMs,
    invoicesSyncIntervalMs: settings.invoicesSyncIntervalMs,
    maxDetailFetches: settings.invoicesMaxDetailFetches,
  });
}
