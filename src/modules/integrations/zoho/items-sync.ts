...
export class SyncAlreadyRunningError extends Error {
  constructor() {
    super('A items sync is already running in this instance');
...
export class SyncFailedError extends Error {
  constructor(
    ...
    super(`Items sync failed with code ${errorCode}`);
    this.name = 'SyncFailedError';
  }
}

export const syncItemsOptionsSchema = z.object({
  mode: z.enum(['scan', 'sync']).default('scan'),
  maxDetailFetches: z.number().int().min(1).max(200).default(50),
});

export type SyncItemsOptions = z.input<typeof syncItemsOptionsSchema>;
export type SyncMode = z.output<typeof syncItemsOptionsSchema>['mode'];
...
const SYNC_LOCK_KEY = '__unikZohoItemsSyncLock' as const;
...
const itemSummarySchema = z.object({
  item_id: z.union([z.string().min(1), z.number()]).transform((value) => String(value)),
...
const itemDetailTimestampSchema = z.object({
  item: z.object({ last_modified_time: remoteTimestampSchema }),
});

function extractDetailModifiedAt(rawDetail: unknown): Date | null {
  const parsed = itemDetailTimestampSchema.safeParse(rawDetail);
  return parsed.success ? parsed.data.item.last_modified_time : null;
}
...
async function scanAllPages(params: SyncParams): Promise<ScanOutcome> {
  ...
  for (;;) {
    ...
    const rawPage = await listItems({ page, perPage: params.perPage });
    apiCalls += 1;

    const parsed = itemsListSchema.safeParse(rawPage);
    if (!parsed.success) {
    ...
    for (const summary of parsed.data.items) {
      await recordSummary(summary.item_id, summary.last_modified_time);
      recordsSeen += 1;
    }
...
async function fetchPendingDetails(
  ...
  for (const entity of pending) {
    ...
    try {
      const rawDetail = await getItem(entity.externalId);
      apiCalls += 1;
...
/**
 * Runs one controlled Items synchronization.
 *
 ...
 */
export async function syncItems(options: SyncItemsOptions = {}): Promise<SyncItemsResult> {
  const { mode, maxDetailFetches } = syncItemsOptionsSchema.parse(options);
  ...
  try {
    ...
    if (mode === 'sync') {
      try {
        const { normalizePendingProductSnapshots } = await import('@/modules/sales/products-normalizer');
        await withTimeout(normalizePendingProductSnapshots({ limit: 100 }), 30_000);
      } catch (error) {
        console.error(
          JSON.stringify({
            event: 'zoho.items.sync.normalization_failed',
            runId: run.id,
    ...
    console.info(
      JSON.stringify({
        event: 'zoho.items.sync.completed',
        runId: run.id,
        mode,
        pagesScanned: result.pagesScanned,
        recordsSeen: result.recordsSeen,
