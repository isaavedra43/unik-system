...
import { getCustomerPayment, listCustomerPayments } from './payments';
import {
...
export const ENTITY_TYPE = 'customerpayment';
...
export class SyncAlreadyRunningError extends Error {
  constructor() {
    super('A customerpayments sync is already running in this instance');
...
export class SyncFailedError extends Error {
  constructor(
    ...
    super(`CustomerPayments sync failed with code ${errorCode}`);
    this.name = 'SyncFailedError';
  }
}

export const syncCustomerPaymentsOptionsSchema = z.object({
  mode: z.enum(['scan', 'sync']).default('scan'),
  maxDetailFetches: z.number().int().min(1).max(200).default(50),
});

export type SyncCustomerPaymentsOptions = z.input<typeof syncCustomerPaymentsOptionsSchema>;
export type SyncMode = z.output<typeof syncCustomerPaymentsOptionsSchema>['mode'];

export interface SyncCustomerPaymentsResult {
  runId: string;
  mode: SyncMode;
  pagesScanned: number;
  recordsSeen: number;
...
const SYNC_LOCK_KEY = '__unikZohoCustomerPaymentsSyncLock' as const;
...
const customerpaymentSummarySchema = z.object({
  payment_id: z.union([z.string().min(1), z.number()]).transform((value) => String(value)),
  last_modified_time: remoteTimestampSchema,
});

const customerpaymentsListSchema = z.object({
  customerpayments: z.array(customerpaymentSummarySchema),
  page_context: z
    .object({
...
const customerpaymentDetailTimestampSchema = z.object({
  customerpayment: z.object({ last_modified_time: remoteTimestampSchema }),
});

function extractDetailModifiedAt(rawDetail: unknown): Date | null {
  const parsed = customerpaymentDetailTimestampSchema.safeParse(rawDetail);
  return parsed.success ? parsed.data.customerpayment.last_modified_time : null;
}
...
async function scanAllPages(params: SyncParams): Promise<ScanOutcome> {
  ...
  for (;;) {
    ...
    const rawPage = await listCustomerPayments({ page, perPage: params.perPage });
    apiCalls += 1;

    const parsed = customerpaymentsListSchema.safeParse(rawPage);
    if (!parsed.success) {
    ...
    for (const summary of parsed.data.customerpayments) {
      await recordSummary(summary.payment_id, summary.last_modified_time);
      recordsSeen += 1;
    }
...
async function fetchPendingDetails(
  ...
  for (const entity of pending) {
    ...
    try {
      const rawDetail = await getCustomerPayment(entity.externalId);
      apiCalls += 1;
...
/**
 * Runs one controlled CustomerPayments synchronization.
 *
 ...
 */
export async function syncCustomerPayments(options: SyncCustomerPaymentsOptions = {}): Promise<SyncCustomerPaymentsResult> {
  const { mode, maxDetailFetches } = syncCustomerPaymentsOptionsSchema.parse(options);
  ...
  const timeoutId = setTimeout(() => {
    ...
    console.error(
      JSON.stringify({
        event: 'zoho.customerpayments.sync.timeout',
  ...
  try {
    ...
    if (mode === 'sync') {
      try {
        const { normalizePendingPaymentSnapshots } = await import('@/modules/sales/invoices-normalizer');
        await withTimeout(normalizePendingPaymentSnapshots({ limit: 100 }), 30_000);
      } catch (error) {
        console.error(
          JSON.stringify({
            event: 'zoho.customerpayments.sync.normalization_failed',
            runId: run.id,
    ...
    const result: SyncCustomerPaymentsResult = {
      runId: run.id,
      mode,
    ...
    console.info(
      JSON.stringify({
        event: 'zoho.customerpayments.sync.completed',
        runId: run.id,
        mode,
        pagesScanned: result.pagesScanned,
        recordsSeen: result.recordsSeen,
    ...
    console.error(
      JSON.stringify({
        event: 'zoho.customerpayments.sync.failed',
        runId: run?.id ?? 'unknown',
        mode,
        errorCode,
        durationMs: completedAt.getTime() - startedAt.getTime(),
