 ...
 * packagesSyncIntervalMs. IntegrationSyncRun is the durable source of truth, so a
...
const SCHEDULER_STATE_KEY = '__unikZohoPackagesSchedulerState' as const;
 ...
 * one is older than packagesSyncIntervalMs. Only `mode = 'sync'` runs count: baseline
...
export async function isSyncDue(now: Date = new Date()): Promise<boolean> {
  ...
  return now.getTime() - lastCompletedSync.completedAt.getTime() >= settings.packagesSyncIntervalMs;
}
...
export async function runSchedulerCheck(): Promise<void> {
  ...
  try {
    ...
    log({
      event: 'zoho.packages.scheduler.tick_skipped',
  ...
  try {
    ...
    if (await isSyncCoolingDown()) {
      log({
        event: 'zoho.packages.scheduler.sync_skipped',
        reason: 'failed_retry_cooldown',
      });
      return;
    }

    log({ event: 'zoho.packages.scheduler.due' });
    const startedAt = Date.now();
    const settings = await getIntegrationSettings(INTEGRATION_SOURCE_ZOHO);

    if (!settings.packagesSyncEnabled) {
      return;
    }

    log({ event: 'zoho.packages.scheduler.sync_started' });

    const result = await syncPackages({
      mode: 'sync',
      maxDetailFetches: settings.packagesMaxDetailFetches,
    });

    log({
      event: 'zoho.packages.scheduler.sync_completed',
      runId: result.runId,
    ...
    log({
      event: 'zoho.packages.scheduler.sync_failed',
 ...
 */
export async function startPackagesScheduler(): Promise<void> {
  const state = getSchedulerState();
  ...
  try {
    ...
    log({
      event: 'zoho.packages.scheduler.start_failed',
      reason: error instanceof Error ? error.message : 'db_unreachable',
    });
    return;
  }

  if (!enabled || !settings.packagesSyncEnabled) {
    log({ event: 'zoho.packages.scheduler.disabled' });
    return;
  }
  ...
  log({
    event: 'zoho.packages.scheduler.started',
    startupDelayMs: settings.startupDelayMs,
    checkIntervalMs: settings.checkIntervalMs,
    packagesSyncIntervalMs: settings.packagesSyncIntervalMs,
    maxDetailFetches: settings.packagesMaxDetailFetches,
  });
}
