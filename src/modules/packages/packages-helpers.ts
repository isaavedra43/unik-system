...
const STATUS_MAP: Record<string, PackageStatusConfig> = {
...
export function getPackageStatusConfig(raw: string | null | undefined): PackageStatusConfig {
  const normalized = normalizeRaw(raw);
  if (!normalized) return { raw: raw ?? '', label: '—', tone: 'muted' };
  return STATUS_MAP[normalized] ?? { raw: raw ?? '', label: humanizeRaw(raw), tone: 'muted' };
}

export function getPackageStatusLabel(raw: string | null | undefined): string {
  return getPackageStatusConfig(raw).label;
}

export function getPackageStatusOptions(): { value: string; label: string }[] {
