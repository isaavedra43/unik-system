import { z } from 'zod';
import { PACKAGE_FILTERABLE_FIELDS, PACKAGE_SORTABLE_FIELDS } from './packages-columns';

export const TEXT_OPERATORS = [
  'contains', 'not_contains', 'equals', 'not_equals', 'starts_with', 'is_empty', 'is_not_empty',
] as const;
...
const textFilterSchema = z.object({
  field: z.string().refine((f) => PACKAGE_FILTERABLE_FIELDS.has(f)),
...
export const packageFilterGroupSchema = z.object({
  ...
  rules: z.array(packageFilterRuleSchema).default([]),
});
...
export const packageQueryStateSchema = z.object({
  ...
  filters: packageFilterGroupSchema.default({ logic: 'AND', rules: [] }),
  sort: packageSortSchema.default([]),
  page: z.coerce.number().int().min(1).default(1),
  page_size: z.coerce.number().int().min(1).max(500).default(50),
});
export type PackageQueryState = z.output<typeof packageQueryStateSchema>;
