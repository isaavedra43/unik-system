...
import {
  billTablePreferenceConfigSchema,
  billTableViewVisibilitySchema,
  type BillTablePreferenceConfig,
} from '@/modules/bills/bills-filters';
import { BILLS_TABLE_KEY } from '@/modules/bills/bills-columns';
import { BILL_ENTITY_TYPE } from '@/modules/bills/permissions';
import { recordAuditEvent } from '@/modules/auth/audit-service';
import { formatDateOnly, getBillStatusConfig } from '@/modules/bills/bills-helpers';

const BILLS_PATH = '/app/bills';
...
export async function saveTablePreferenceJson(
  config: BillTablePreferenceConfig
): Promise<{ error: string | null; success: boolean }> {
  try {
    const actor = await requireActor('bills.view');
    const parsed = billTablePreferenceConfigSchema.parse(config);
    await upsertUserTablePreference(actor.id, BILLS_TABLE_KEY, parsed);
...
export async function resetTablePreferenceAction(): Promise<{ error: string | null; success: boolean }> {
  try {
    const actor = await requireActor('bills.view');
    await deleteUserTablePreference(actor.id, BILLS_TABLE_KEY);
...
const createViewSchema = z.object({
  ...
  visibility: billTableViewVisibilitySchema.default('private'),
  config: z.string(),
...
export async function watchAction(
  ...
  try {
    const actor = await requireActor('bills.watch');
    ...
    await watchEntity(actor, BILL_ENTITY_TYPE, parsed.data.entityId);
    revalidatePath(BILLS_PATH);
...
export async function unwatchAction(
  ...
  try {
    ...
    await unwatchEntity(actor, BILL_ENTITY_TYPE, parsed.data.entityId);
    revalidatePath(BILLS_PATH);
...
export async function bulkWatchAction(
  ...
  try {
    ...
    if (parsed.data.action === 'watch') {
      await bulkWatchEntities(actor, BILL_ENTITY_TYPE, parsed.data.entityIds);
    } else {
      await bulkUnwatchEntities(actor, BILL_ENTITY_TYPE, parsed.data.entityIds);
    }
    revalidatePath(BILLS_PATH);
...
export async function exportAction(
  ...
  try {
    const actor = await requireActor('bills.export');
    ...
    const { getBillsForExport, buildCsv } = await import('@/modules/bills/bills-service');
    const { rows, columns } = await getBillsForExport(query, {
    ...
    if (parsed.data.format === 'csv') {
      ...
      return {
        ...
        success: true,
        content,
        filename: `facturas-de-compra-${timestamp}.csv`,
        format: 'csv',
      };
    }
    ...
    for (const row of rows) {
      ...
      for (const col of columns) {
        ...
        if (val === null || val === undefined) {
          ...
          rowData[col.id] = getBillStatusConfig(val as string | null).label;
        } else {
    ...
    return {
      ...
      success: true,
      content,
      filename: `facturas-de-compra-${timestamp}.xlsx`,
      format: 'xlsx',
    };
  } catch (error) {
