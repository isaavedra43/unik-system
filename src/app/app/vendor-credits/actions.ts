...
import { VENDOR_CREDITS_TABLE_KEY } from '@/modules/vendor-credits/vendor-credits-columns';
...
const VENDOR_CREDITS_PATH = '/app/vendor-credits';
...
export async function saveTablePreferenceJson(
  ...
  try {
    const actor = await requireActor('vendor_credits.view');
    const parsed = tablePreferenceConfigSchema.parse(config);
    await upsertUserTablePreference(actor.id, VENDOR_CREDITS_TABLE_KEY, parsed);
...
export async function resetTablePreferenceAction(): Promise<{ error: string | null; success: boolean }> {
  try {
    const actor = await requireActor('vendor_credits.view');
    await deleteUserTablePreference(actor.id, VENDOR_CREDITS_TABLE_KEY);
    revalidatePath(VENDOR_CREDITS_PATH);
    return { error: null, success: true };
  } catch (error) {
    return { error: errorMessage(error), success: false };
  }
...
export async function watchAction(
  ...
  try {
    const actor = await requireActor('vendor_credits.watch');
    ...
    await watchEntity(actor, VENDOR_CREDIT_ENTITY_TYPE, parsed.data.entityId);
    revalidatePath(VENDOR_CREDITS_PATH);
...
export async function unwatchAction(
  ...
  try {
    ...
    await unwatchEntity(actor, VENDOR_CREDIT_ENTITY_TYPE, parsed.data.entityId);
    revalidatePath(VENDOR_CREDITS_PATH);
...
export async function bulkWatchAction(
  ...
  try {
    ...
    if (parsed.data.action === 'watch') {
      await bulkWatchEntities(actor, VENDOR_CREDIT_ENTITY_TYPE, parsed.data.entityIds);
    } else {
      await bulkUnwatchEntities(actor, VENDOR_CREDIT_ENTITY_TYPE, parsed.data.entityIds);
    }
    revalidatePath(VENDOR_CREDITS_PATH);
...
export async function exportAction(
  ...
  try {
    const actor = await requireActor('vendor_credits.export');
    ...
    const {
      getVendorCreditsForExport,
      buildCsv,
    } = await import('@/modules/vendor-credits/vendor-credits-service');
    const { rows, columns } = await getVendorCreditsForExport(query, {
    ...
    if (parsed.data.format === 'csv') {
      ...
      await recordAuditEvent({
        ...
        action: 'vendor_credits.exported',
      ...
      return {
        ...
        success: true,
        content,
        filename: `creditos-proveedor-${timestamp}.csv`,
        format: 'csv',
      };
    }
    ...
    const sheet = workbook.addWorksheet('Créditos de proveedor');
    ...
    for (const row of rows) {
      ...
      for (const col of columns) {
        ...
        if (val === null || val === undefined) {
          ...
          rowData[col.id] = getVendorCreditStatusConfig(val as string | null).label;
        } else {
    ...
    return {
      ...
      success: true,
      content,
      filename: `creditos-proveedor-${timestamp}.xlsx`,
      format: 'xlsx',
    };
  } catch (error) {
