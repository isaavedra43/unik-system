...
import { PURCHASE_ORDERS_TABLE_KEY } from '@/modules/purchase-orders/purchase-orders-columns';
...
const PURCHASE_ORDERS_PATH = '/app/purchase-orders';
...
export async function saveTablePreferenceJson(
  ...
  try {
    const actor = await requireActor('purchase_orders.view');
    const parsed = tablePreferenceConfigSchema.parse(config);
    await upsertUserTablePreference(actor.id, PURCHASE_ORDERS_TABLE_KEY, parsed);
...
export async function resetTablePreferenceAction(): Promise<{
  error: string | null;
  success: boolean;
}> {
  try {
    const actor = await requireActor('purchase_orders.view');
    await deleteUserTablePreference(actor.id, PURCHASE_ORDERS_TABLE_KEY);
...
export async function watchAction(
  ...
  try {
    const actor = await requireActor('purchase_orders.watch');
    ...
    await watchEntity(actor, PURCHASE_ORDER_ENTITY_TYPE, parsed.data.entityId);
    revalidatePath(PURCHASE_ORDERS_PATH);
...
export async function unwatchAction(
  ...
  try {
    ...
    await unwatchEntity(actor, PURCHASE_ORDER_ENTITY_TYPE, parsed.data.entityId);
    revalidatePath(PURCHASE_ORDERS_PATH);
...
export async function bulkWatchAction(
  ...
  try {
    ...
    if (parsed.data.action === 'watch') {
      await bulkWatchEntities(actor, PURCHASE_ORDER_ENTITY_TYPE, parsed.data.entityIds);
    } else {
      await bulkUnwatchEntities(actor, PURCHASE_ORDER_ENTITY_TYPE, parsed.data.entityIds);
    }
    revalidatePath(PURCHASE_ORDERS_PATH);
...
export async function exportAction(
  ...
  try {
    const actor = await requireActor('purchase_orders.export');
    ...
    const { getPurchaseOrdersForExport, buildCsv } = await import(
      '@/modules/purchase-orders/purchase-orders-service'
    );
    const { rows, columns } = await getPurchaseOrdersForExport(query, {
    ...
    if (parsed.data.format === 'csv') {
      ...
      await recordAuditEvent({
        ...
        action: 'purchase_orders.exported',
      ...
      return {
        ...
        success: true,
        content,
        filename: `ordenes-compra-${timestamp}.csv`,
        format: 'csv',
      };
    }
    ...
    for (const row of rows) {
      ...
      for (const col of columns) {
        ...
        if (val === null || val === undefined) {
          ...
          rowData[col.id] = getPurchaseOrderStatusConfig(val as string | null).label;
        } else {
    ...
    return {
      ...
      success: true,
      content,
      filename: `ordenes-compra-${timestamp}.xlsx`,
      format: 'xlsx',
    };
  } catch (error) {
    return {
