...
export async function POST() {
  ...
  try {
    ...
    const syncPromise = syncPurchaseOrders({ mode: 'quick', maxDetailFetches: 30 });

    after(async () => {
      try {
        await syncPromise;
      } catch {
        // Errors are already logged inside syncPurchaseOrders.
      }
    });
    ...
    console.error('purchase orders sync trigger error', error);
