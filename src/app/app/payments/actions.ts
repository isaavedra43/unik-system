...
import { PAYMENTS_TABLE_KEY } from '@/modules/payments/payments-columns';
import { PAYMENT_ENTITY_TYPE } from '@/modules/payments/permissions';
import { recordAuditEvent } from '@/modules/auth/audit-service';
import { formatDateOnly, getPaymentStatusConfig } from '@/modules/payments/payments-helpers';
...
export async function saveTablePreferenceJson(
  ...
  try {
    const actor = await requireActor('payments.view');
    const parsed = tablePreferenceConfigSchema.parse(config);
    await upsertUserTablePreference(actor.id, PAYMENTS_TABLE_KEY, parsed);
...
export async function resetTablePreferenceAction(): Promise<{ error: string | null; success: boolean }> {
  try {
    const actor = await requireActor('payments.view');
    await deleteUserTablePreference(actor.id, PAYMENTS_TABLE_KEY);
...
export async function watchAction(
  ...
  try {
    const actor = await requireActor('payments.watch');
    ...
    await watchEntity(actor, PAYMENT_ENTITY_TYPE, parsed.data.entityId);
    revalidatePath(PAYMENTS_PATH);
...
export async function unwatchAction(
  ...
  try {
    ...
    await unwatchEntity(actor, PAYMENT_ENTITY_TYPE, parsed.data.entityId);
    revalidatePath(PAYMENTS_PATH);
...
export async function bulkWatchAction(
  ...
  try {
    ...
    if (parsed.data.action === 'watch') {
      await bulkWatchEntities(actor, PAYMENT_ENTITY_TYPE, parsed.data.entityIds);
    } else {
      await bulkUnwatchEntities(actor, PAYMENT_ENTITY_TYPE, parsed.data.entityIds);
    }
    revalidatePath(PAYMENTS_PATH);
...
export async function exportAction(
  ...
  try {
    const actor = await requireActor('payments.export');
    ...
    const { getPaymentsForExport, buildCsv } = await import('@/modules/payments/payments-service');
    const { rows, columns } = await getPaymentsForExport(query, {
    ...
    for (const row of rows) {
      ...
      for (const col of columns) {
        ...
        if (val === null || val === undefined) {
          ...
          rowData[col.id] = getPaymentStatusConfig(val as string | null).label;
        } else {
