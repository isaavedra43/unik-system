...
export async function POST() {
  ...
  try {
    ...
    const syncPromise = syncPayments({ mode: 'quick', maxDetailFetches: 30 });

    after(async () => {
      try {
        await syncPromise;
      } catch {
        // Errors are already logged inside syncPayments.
      }
    });
    ...
    console.error('payments sync trigger error', error);
