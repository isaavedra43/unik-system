...
import { CONTACTS_TABLE_KEY_CUSTOMERS } from '@/modules/contacts/contacts-columns';
...
const CUSTOMERS_PATH = '/app/contacts/customers';
...
export async function saveTablePreferenceJson(
  ...
  try {
    const actor = await requireActor('customers.view');
    const parsed = tablePreferenceConfigSchema.parse(config);
    await upsertUserTablePreference(actor.id, CONTACTS_TABLE_KEY_CUSTOMERS, parsed);
...
export async function resetTablePreferenceAction(): Promise<{ error: string | null; success: boolean }> {
  try {
    const actor = await requireActor('customers.view');
    await deleteUserTablePreference(actor.id, CONTACTS_TABLE_KEY_CUSTOMERS);
...
export async function watchAction(
  ...
  try {
    const actor = await requireActor('customers.watch');
    ...
    await watchEntity(actor, CONTACT_ENTITY_TYPE_CUSTOMER, parsed.data.entityId);
    revalidatePath(CUSTOMERS_PATH);
...
export async function unwatchAction(
  ...
  try {
    ...
    await unwatchEntity(actor, CONTACT_ENTITY_TYPE_CUSTOMER, parsed.data.entityId);
    revalidatePath(CUSTOMERS_PATH);
...
export async function bulkWatchAction(
  ...
  try {
    ...
    if (parsed.data.action === 'watch') {
      await bulkWatchEntities(actor, CONTACT_ENTITY_TYPE_CUSTOMER, parsed.data.entityIds);
    } else {
      await bulkUnwatchEntities(actor, CONTACT_ENTITY_TYPE_CUSTOMER, parsed.data.entityIds);
    }
    revalidatePath(CUSTOMERS_PATH);
...
export async function exportAction(
  ...
  try {
    const actor = await requireActor('customers.export');
    ...
    const { rows, columns } = await getContactsForExport(query, 'customer', {
