...
export async function POST() {
  ...
  try {
    const activeRun = await getActiveEstimatesSyncRun();
    if (activeRun)
      return NextResponse.json({ already_running: true, run_id: activeRun.runId }, { status: 409 });

    const syncPromise = syncEstimates({ mode: 'quick', maxDetailFetches: 30 });

    after(async () => {
      try {
        await syncPromise;
      } catch {
        // Errors are already logged inside syncEstimates.
      }
    });
    ...
    console.error('quotes sync trigger error', error);
