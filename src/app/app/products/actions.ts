...
import { PRODUCTS_TABLE_KEY } from '@/modules/products/products-columns';
import { PRODUCT_ENTITY_TYPE } from '@/modules/products/permissions';
import { recordAuditEvent } from '@/modules/auth/audit-service';
import { formatDateOnly, getProductStatusConfig } from '@/modules/products/products-helpers';
...
export async function saveTablePreferenceJson(
  ...
  try {
    const actor = await requireActor('products.view');
    const parsed = tablePreferenceConfigSchema.parse(config);
    await upsertUserTablePreference(actor.id, PRODUCTS_TABLE_KEY, parsed);
...
export async function resetTablePreferenceAction(): Promise<{ error: string | null; success: boolean }> {
  try {
    const actor = await requireActor('products.view');
    await deleteUserTablePreference(actor.id, PRODUCTS_TABLE_KEY);
...
export async function watchAction(
  ...
  try {
    const actor = await requireActor('products.watch');
    ...
    await watchEntity(actor, PRODUCT_ENTITY_TYPE, parsed.data.entityId);
    revalidatePath(PRODUCTS_PATH);
...
export async function unwatchAction(
  ...
  try {
    ...
    await unwatchEntity(actor, PRODUCT_ENTITY_TYPE, parsed.data.entityId);
    revalidatePath(PRODUCTS_PATH);
...
export async function bulkWatchAction(
  ...
  try {
    ...
    if (parsed.data.action === 'watch') {
      await bulkWatchEntities(actor, PRODUCT_ENTITY_TYPE, parsed.data.entityIds);
    } else {
      await bulkUnwatchEntities(actor, PRODUCT_ENTITY_TYPE, parsed.data.entityIds);
    }
    revalidatePath(PRODUCTS_PATH);
...
export async function exportAction(
  ...
  try {
    const actor = await requireActor('products.export');
    ...
    const { getProductsForExport, buildCsv } = await import('@/modules/products/products-service');
    const { rows, columns } = await getProductsForExport(query, {
    ...
    if (parsed.data.format === 'csv') {
      ...
      return {
        ...
        success: true,
        content,
        filename: `productos-${timestamp}.csv`,
        format: 'csv',
      };
    }
    ...
    for (const row of rows) {
      ...
      for (const col of columns) {
        ...
        if (val === null || val === undefined) {
          ...
          rowData[col.id] = getProductStatusConfig(val as string | null).label;
        } else {
    ...
    return {
      ...
      success: true,
      content,
      filename: `productos-${timestamp}.xlsx`,
      format: 'xlsx',
    };
  } catch (error) {
