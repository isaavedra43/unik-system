...
import { PRODUCTS_TABLE_KEY, PRODUCT_COLUMNS } from '@/modules/products/products-columns';
import {
...
export default async function ProductsPage({
  ...
  const query = productQueryStateSchema.parse(queryInput);

  const [result, preference, views, defaultView, unreadCount, activeSyncRun, latestSyncRun] =
    await Promise.all([
      getProductsWorkspace(query),
      getUserTablePreference(user.id, PRODUCTS_TABLE_KEY),
      listTableViews(user.id, PRODUCTS_TABLE_KEY),
      getDefaultTableView(user.id, PRODUCTS_TABLE_KEY),
  ...
  const canExport = hasPermission(user, 'products.export');
  const canWatch = hasPermission(user, 'products.watch');
  const canShareViews = hasPermission(user, 'products.share_views');

  const defaultPreference: TablePreferenceConfig = {
    version: 1,
    columnOrder: PRODUCT_COLUMNS.sort((a, b) => a.priority - b.priority).map((c) => c.id),
    columnVisibility: Object.fromEntries(PRODUCT_COLUMNS.map((c) => [c.id, c.defaultVisible])),
    columnWidths: Object.fromEntries(PRODUCT_COLUMNS.map((c) => [c.id, c.defaultWidth])),
  ...
  return (
    <ProductsWorkspace
      ...
      permissionExport="products.export"
      permissionWatch="products.watch"
      permissionShareViews="products.share_views"
      initialData={result}
      initialQuery={query}
