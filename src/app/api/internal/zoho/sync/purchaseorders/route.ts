...
import {
  startSyncPurchaseOrders, getActivePurchaseOrdersSyncRun, getLatestPurchaseOrdersSyncRun,
...
export async function POST(request: Request) {
  ...
  try {
    if (mode === 'baseline') {
      const { baselinePurchaseOrders } = await import('@/modules/integrations/zoho/purchase-orders-sync');
      const result = await baselinePurchaseOrders();
      return NextResponse.json({ result });
    }
    const activeRun = await getActivePurchaseOrdersSyncRun();
    if (activeRun) return NextResponse.json({ already_running: true, run_id: activeRun.runId }, { status: 409 });
    const result = await startSyncPurchaseOrders({ mode: 'sync' });
    ...
    console.error('internal purchase orders sync error', error);
...
export async function GET() {
  ...
  if (!session.user.isSuperAdmin && !session.user.permissionKeys.includes('integrations.view'))
    return NextResponse.json({ error: 'Sin permiso' }, { status: 403 });
  const [latestRun, activeRun] = await Promise.all([getLatestPurchaseOrdersSyncRun(), getActivePurchaseOrdersSyncRun()]);
