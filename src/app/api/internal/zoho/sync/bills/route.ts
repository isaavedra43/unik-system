...
import {
  startSyncBills, getActiveBillsSyncRun, getLatestBillsSyncRun,
...
export async function POST(request: Request) {
  ...
  try {
    if (mode === 'baseline') {
      const { baselineBills } = await import('@/modules/integrations/zoho/bills-sync');
      const result = await baselineBills();
      return NextResponse.json({ result });
    }
    const activeRun = await getActiveBillsSyncRun();
    if (activeRun) return NextResponse.json({ already_running: true, run_id: activeRun.runId }, { status: 409 });
    const result = await startSyncBills({ mode: 'sync' });
    ...
    console.error('internal bills sync error', error);
...
export async function GET() {
  ...
  if (!session.user.isSuperAdmin && !session.user.permissionKeys.includes('integrations.view'))
    return NextResponse.json({ error: 'Sin permiso' }, { status: 403 });
  const [latestRun, activeRun] = await Promise.all([getLatestBillsSyncRun(), getActiveBillsSyncRun()]);
