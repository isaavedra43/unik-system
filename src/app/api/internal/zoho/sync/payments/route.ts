...
import {
  startSyncPayments, getActivePaymentsSyncRun, getLatestPaymentsSyncRun,
...
export async function POST(request: Request) {
  ...
  try {
    if (mode === 'baseline') {
      const { baselinePayments } = await import('@/modules/integrations/zoho/payments-sync');
      const result = await baselinePayments();
      return NextResponse.json({ result });
    }
    const activeRun = await getActivePaymentsSyncRun();
    if (activeRun) return NextResponse.json({ already_running: true, run_id: activeRun.runId }, { status: 409 });
    const result = await startSyncPayments({ mode: 'sync' });
    ...
    console.error('internal payments sync error', error);
