...
import {
  startSyncVendorCredits, getActiveVendorCreditsSyncRun, getLatestVendorCreditsSyncRun,
...
export async function POST(request: Request) {
  ...
  try {
    if (mode === 'baseline') {
      const { baselineVendorCredits } = await import('@/modules/integrations/zoho/vendor-credits-sync');
      const result = await baselineVendorCredits();
    ...
    const result = await startSyncVendorCredits({ mode: 'sync' });
    ...
    console.error('internal vendor credits sync error', error);
...
export async function GET() {
  ...
  if (!session.user.isSuperAdmin && !session.user.permissionKeys.includes('integrations.view'))
    return NextResponse.json({ error: 'Sin permiso' }, { status: 403 });
  const [latestRun, activeRun] = await Promise.all([getLatestVendorCreditsSyncRun(), getActiveVendorCreditsSyncRun()]);
