...
import {
  NormalizationAlreadyRunningError, normalizePendingPackageSnapshots,
} from '@/modules/packages/packages-normalizer';
...
export async function POST(request: Request) {
  ...
  try {
    const result = await normalizePendingPackageSnapshots({ limit });
    ...
    console.error('internal packages normalize error', error);
