...
import {
  NormalizationAlreadyRunningError, normalizePendingPaymentSnapshots,
} from '@/modules/payments/payments-normalizer';
...
export async function POST(request: Request) {
  ...
  try {
    const result = await normalizePendingPaymentSnapshots({ limit });
    ...
    console.error('internal payments normalize error', error);
